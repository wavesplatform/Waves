...
class Sha256Specification  extends FunSuite with Matchers {
  ...
  test("hash comparing with externally computed value") {
    ...
    assert(bytes2hex(hash("".getBytes)) == "e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855")
    assert(bytes2hex(hash("abc".getBytes)) == "ba7816bf8f01cfea414140de5dae2223b00361a396177a9cb410ff61f20015ad")
    assert(bytes2hex(hash("abcdbcdecdefdefgefghfghighijhijkijkljklmklmnlmnomnopnopq".getBytes)) ==
