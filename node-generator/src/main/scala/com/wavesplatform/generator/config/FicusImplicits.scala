...
import com.wavesplatform.transaction.{TransactionParserLite, TransactionParsers}
...
trait FicusImplicits {
  implicit val distributionsReader: ValueReader[Map[TransactionParserLite, Double]] = {
    ...
    def toTxType(key: String): TransactionParserLite = TransactionParsers.by(converter.convert(key)).get
