...
object StateSyntheticBenchmark {
  ...
  class St extends BaseState {
    protected override def txGenP(sender: KeyPair, ts: Long): Gen[Transaction] =
      ...
      } yield TransferTransaction.selfSigned(1.toByte, ts, sender, recipient, Waves, amount, Waves, 100000, Array.emptyByteArray).explicitGet()
  }
  ...
  class SmartSt extends BaseState {
    ...
    protected override def txGenP(sender: KeyPair, ts: Long): Gen[Transaction] =
      for {
        ...
        TransferTransaction
          .selfSigned(2.toByte, ts, sender, recipient.toAddress, Waves, amount, Waves, 1000000, Array.emptyByteArray)
          .explicitGet()
