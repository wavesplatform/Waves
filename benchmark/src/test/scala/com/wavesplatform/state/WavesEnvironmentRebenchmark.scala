...
import com.wavesplatform.transaction.{DataTransaction, Succeeded}
        ...
        .flatMap { h =>
          ...
          if (txCount == 0)
            ...
            db.get(Keys.transactionAt(Height(h), TxNum(Random.nextInt(txCount).toShort)))
              .collect { case (dataTx: DataTransaction, Succeeded) if dataTx.data.nonEmpty =>
                (
      ...
      1.to(environment.height.toInt, 100)
        .flatMap { h =>
          ...
          if (txCount == 0)
            ...
            db.get(Keys.transactionAt(Height(h), TxNum(Random.nextInt(txCount).toShort)))
              .collect { case (transferTx: TransferTransaction, Succeeded) => transferTx.id() }
        }
    }
