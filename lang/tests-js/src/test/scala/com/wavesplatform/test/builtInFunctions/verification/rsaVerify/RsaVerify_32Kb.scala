...
object RsaVerify_32Kb extends JsTestBase {
  ...
  val tests: Tests = Tests {
    test.apply("check: rsaVerify_32Kb function compiles with a ByteVector") {
    ...
    test.apply("check: rsaVerify_32Kb function compiles with a ByteVector(argument before function)") {
      for (version <- testData.actualVersionsWithoutV3) {
        val precondition = new GeneratorContractsForBuiltInFunctions("Boolean", version)
    ...
    test.apply("compilation error: rsaVerify_32Kb - Non-matching types: expected: ByteVector") {
      for (version <- testData.actualVersionsWithoutV3) {
    ...
    test.apply("compilation error: rsaVerify_32Kb - Non-matching types: expected: ByteVector (argument before function)") {
      for (version <- testData.actualVersionsWithoutV3) {
        val precondition = new GeneratorContractsForBuiltInFunctions("ByteVector", version)
    ...
    test.apply("compilation error: Can't find a function overload rsaVerify_32Kb") {
      for (version <- testData.actualVersionsWithoutV3) {
        val precondition = new GeneratorContractsForBuiltInFunctions("ByteVector", version)
    ...
    test.apply("compilation error: Can't find a function overload rsaVerify_32Kb (argument before function)") {
      for (version <- testData.actualVersionsWithoutV3) {
        val precondition = new GeneratorContractsForBuiltInFunctions("ByteVector", version)
    ...
    test.apply("compilation error: Can't find a function rsaVerify_32Kb") {
