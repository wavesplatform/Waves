...
object ReentrantInvoke extends JsTestBase {
  ...
  val tests: Tests = Tests {
    test("RIDE-90. ReentrantInvoke function should compile for Issue V4 and more") {
    ...
    test("RIDE-91. ReentrantInvoke function throws an error for invalid values for V4 and more") {
      for (version <- versionsSupportingTheNewFeatures) {
        val precondition = new GeneratorContractsForBuiltInFunctions("", version)
        for (
