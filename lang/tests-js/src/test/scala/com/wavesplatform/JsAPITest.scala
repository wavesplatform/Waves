  ...
  val tests: Tests = Tests {
    ...
    test("benchmark") {
      ...
      val r1 = (1 to 0).map { _ =>
        ...
        JsAPI.compile(script, 3)
        val end = System.currentTimeMillis()
        (end - start) / 1000d
      }.takeRight(25).sum / 25
      println(s"COMPILE = $r1 s")

      val r2 = (1 to 50).map { _ =>
        val start = System.currentTimeMillis()
        val r = JsAPI.parseAndCompile(script, 3)
        val r2 = r.dAppAst()
        val end = System.currentTimeMillis()
        //println(JSON.stringify(r).length)
        (end - start) / 1000d
      }.takeRight(25).sum / 25
      println(s"PARSE AND COMPILE = $r2 s")
    }
  }
}
