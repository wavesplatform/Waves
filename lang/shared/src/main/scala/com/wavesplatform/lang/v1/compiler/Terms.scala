...
object Terms {
  ...
  object CONST_BYTESTR {
    ...
    def apply(bs: ByteStr, limit: Limit = DataEntrySize): Either[StringError, EVALUATED] =
      Either.cond(
        bs.size <= limit.value,
  ...
  object CONST_STRING {
    def apply(s: String, reduceLimit: Boolean = true): Either[StringError, CONST_STRING] = {
      val limit =
