...
object Types {
  ...
  case object UNIT       extends AUTO_TAGGED_TYPE[Unit](name = "Unit")
  case object LONG       extends AUTO_TAGGED_TYPE[Long](name = "Int")
  case object BYTEVECTOR extends AUTO_TAGGED_TYPE[ByteVector](name = "ByteVector")
  case object BOOLEAN    extends AUTO_TAGGED_TYPE[Boolean](name = "Boolean")
  case object STRING     extends AUTO_TAGGED_TYPE[String](name = "String")
  ...
  case class CASETYPEREF(override val name: String, override val fields: List[(String, TYPE)]) extends AUTO_TAGGED_TYPE[CaseObj](name)

  class UNION(override val l: List[TYPE]) extends AUTO_TAGGED_TYPE[CaseObj](name = "UNION(" ++ l.sortBy(_.toString).mkString("|") ++ ")") {
  ...
  def fromNonable(typeName: TYPEPARAM)(t: Map[TYPEPARAM, TYPE]): Either[String, TYPE] =
    Right(t(typeName) match {
      case UNION(l) =>
        l.filter(_ != UNIT) match {
          case Seq(t) => t
          case l      => new UNION(l)
        }
      case t => t
