...
import com.wavesplatform.lang.v1.Terms.Typed.{EXPR, LET}
...
object EvaluatorV1 extends ExprEvaluator {
  ...
  private def getContext: FF[EvaluationContext] = EitherT.apply[F0, ExecutionError, EvaluationContext](StateT.get[Coeval, EvaluationContext].map(_.asRight[ExecutionError]))
  private def updateContext(f: EvaluationContext => EvaluationContext): FF[Unit] = EitherT.apply[F0, ExecutionError, Unit](StateT.modify[Coeval, EvaluationContext](f).map(_.asRight[ExecutionError]))
  ...
  private def evalBlock(maybeLet: Option[LET], inner: EXPR, tpe: TYPE) = {
    for {
      _ <- writeLog(s"LET: $maybeLet; TYPE: $tpe")
      ctx <- getContext
      result <- maybeLet
        .fold(evalExpr(inner)(tpe.typeInfo))({ case LET(varName, varBlock) =>
          if (lets.get(ctx).get(varName).isDefined) liftL(s"Value '$varName' already defined in the scope")
          else if (funcs.get(ctx).keys.exists(_.name == varName)) liftL(s"Value '$varName' can't be defined because function with such name is predefined")
          else {
            val blockEvaluation = evalExpr(varBlock)(varBlock.tpe.typeInfo)
            val lazyBlock = LazyVal(varBlock.tpe)(toTER(ctx, blockEvaluation))
            updateContext(lets.modify(_)(_.updated(varName, lazyBlock))) *> evalExpr(inner)(tpe.typeInfo)
          }
        })
    } yield result
  }
  ...
  private def evalExpr[T: TypeInfo](t: Typed.EXPR): FF[T] = {
    (t match {
      case Typed.BLOCK(mayBeLet, inner, blockTpe) =>
        writeLog("Evaluating BLOCK") *> evalBlock(mayBeLet, inner, blockTpe) <* writeLog("FINISHED")
