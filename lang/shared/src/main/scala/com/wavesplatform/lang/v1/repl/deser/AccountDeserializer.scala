...
case class AccountDeserializer(chainId: Byte) extends JsonDeserializer[Account] {
  override def deserialize(jsonParser: JsonParser, deserializationContext: DeserializationContext): Account = {
    val pk = Base58.decode(jsonParser.getValueAsString)
    Account(chainId, pk, Base58.encode(address(pk, chainId)))
  }
