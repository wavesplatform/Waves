...
object WavesContext {
  ...
  def build(env: Environment): CTX = {
    ...
    def getdataF(name: String, internalName: Short, dataType: DataType) =
      PredefFunction(name, 100, UNION(dataType.innerType, UNIT), List(("address", addressType.typeRef), ("key", STRING)), internalName) {
    ...
    val txHeightByIdF =
      PredefFunction("transactionHeightById", 100, optionLong, List(("id", BYTEVECTOR)), TRANSACTIONHEIGHTBYID) {
