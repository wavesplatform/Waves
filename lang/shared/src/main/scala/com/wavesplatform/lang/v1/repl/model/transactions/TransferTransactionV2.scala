...
class TransferTransactionV2 extends Nothing with TransferTransaction {
  ...
  def getTransactionMaxSize: Int = TransferTransactionV2.MAX_TX_SIZE
  ...
  override def getType: Byte = TransferTransaction.TRANSFER

  override def getVersion: Byte = Transaction.V2

  def withProof(index: Int, proof: ByteString): TransferTransactionV2 = {
    val newProofs = updateProofs(index, proof)
    new TransferTransactionV2(senderPublicKey, recipient, amount, assetId, fee, feeAssetId, attachment, timestamp, newProofs)
  }

  override def equals(o: Any): Boolean = {
    if (this eq o) return true
    if (o == null || (getClass ne o.getClass)) return false
    val that = o.asInstanceOf[TransferTransactionV2]
