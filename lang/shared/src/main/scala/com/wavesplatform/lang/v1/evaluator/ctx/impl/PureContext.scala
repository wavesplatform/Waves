...
object PureContext {
  ...
  lazy val geInt512: BaseFunction[NoContext] = createInt512ConditionOp(GE_OP, GE_INT512)(_ >= _)
  lazy val gtInt512: BaseFunction[NoContext] = createInt512ConditionOp(GT_OP, GT_INT512)(_ > _)
  ...
  lazy val intToInt512: BaseFunction[NoContext] =
    NativeFunction("toInt512", 1, TO_INT512, INT512, ("n", LONG)) {
      case CONST_LONG(n) :: Nil => Right(CONST_INT512(BigInt(n)))
      case xs                   => notImplemented[Id, EVALUATED]("toInt512(n: Int)", xs)
    }

  lazy val int512ToInt: BaseFunction[NoContext] =
    NativeFunction("toInt", 1, INT512_TO_INT, LONG, ("n", INT512)) {
      case CONST_INT512(n) :: Nil =>
        Either.cond(Long.MaxValue >= n && n >= Long.MinValue, CONST_LONG(n.toLong), s"toInt: Int512 $n out of integers range")
      case xs => notImplemented[Id, EVALUATED]("toInt512(n: Int)", xs)
    }
  ...
  lazy val stringToInt512: BaseFunction[NoContext] =
    NativeFunction("parseInt512Value", 65, STRING_TO_INT512, INT512, ("n", STRING)) {
  ...
  lazy val stringToInt512Opt: BaseFunction[NoContext] =
    NativeFunction("parseInt512", 65, STRING_TO_INT512OPT, UNION(INT512, UNIT), ("n", STRING)) {
      case CONST_STRING(n) :: Nil =>
      ...
      case xs => notImplemented[Id, EVALUATED]("parseInt512(n: String)", xs)
    }
  ...
  def createInt512Op(op: BinaryOperation, func: Short, complexity: Map[StdLibVersion, Long])(
  ...
  lazy val sumToInt512: BaseFunction[NoContext] = createInt512Op(SUM_OP, SUM_INT512, Map[StdLibVersion, Long](V5 -> 8L)) { _ + _ }
  lazy val subToInt512: BaseFunction[NoContext] = createInt512Op(SUB_OP, SUB_INT512, Map[StdLibVersion, Long](V5 -> 8L)) { _ - _ }
  lazy val mulToInt512: BaseFunction[NoContext] = createInt512Op(MUL_OP, MUL_INT512, Map[StdLibVersion, Long](V5 -> 64L)) { _ * _ }
  lazy val divToInt512: BaseFunction[NoContext] = createInt512Op(DIV_OP, DIV_INT512, Map[StdLibVersion, Long](V5 -> 64L)) { _ / _ }
  lazy val modToInt512: BaseFunction[NoContext] = createInt512Op(MOD_OP, MOD_INT512, Map[StdLibVersion, Long](V5 -> 64L)) { _ % _ }
  ...
  val fractionInt512: BaseFunction[NoContext] =
    NativeFunction(
      ...
      case CONST_INT512(v) :: CONST_INT512(n) :: CONST_INT512(d) :: Nil =>
        for {
          _ <- Either.cond(d != 0, (), "Fraction: division by zero")
          result = v * n / d
  ...
  def fractionRounds(roundTypes: UNION): BaseFunction[NoContext] =
    NativeFunction(
      ...
      case CONST_INT512(v) :: CONST_INT512(n) :: CONST_INT512(d) :: (r: CaseObj) :: Nil =>
        for {
          _ <- Either.cond(d != 0, (), "Fraction: division by zero")
          p       = v * n
          s       = p.sign * d.sign
  ...
  def createInt512ConditionOp(op: BinaryOperation, func: Short, complexity: Int = 8)(
      body: (BigInt, BigInt) => Boolean
  ): BaseFunction[NoContext] =
    NativeFunction(opsToFunctions(op), complexity, func, INT512, ("a", INT512), ("b", INT512)) {
      case CONST_INT512(a) :: CONST_INT512(b) :: Nil => Right(CONST_BOOLEAN(body(a, b)))
      case xs                                        => notImplemented[Id, EVALUATED](s"${opsToFunctions(op)}(a: Int512, b: Int512)", xs)
    }
  ...
  lazy val listInt512Max: BaseFunction[NoContext] =
    NativeFunction("max", 192, MAX_LIST_INT512, INT512, ("list", PARAMETERIZEDLIST(INT512))) {
      case ARR(list) :: Nil =>
        Either.cond(
          list.nonEmpty,
          list.asInstanceOf[IndexedSeq[CONST_INT512]].max,
          "Can't find max for empty list"
        )
      case xs =>
  ...
  lazy val listInt512Min: BaseFunction[NoContext] =
    NativeFunction("min", 192, MIN_LIST_INT512, INT512, ("list", PARAMETERIZEDLIST(INT512))) {
      case ARR(list) :: Nil =>
        Either.cond(
          list.nonEmpty,
          list.asInstanceOf[IndexedSeq[CONST_INT512]].min,
          "Can't find min for empty list"
        )
      case xs =>
...
//  }
  ...
  private val v5Types: Seq[REAL]       = commonTypes ++ fromV5RoundTypes ++ Seq(INT512)
