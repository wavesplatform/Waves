...
object ScriptResult {
  ...
  private def processWriteSet(c: CaseObj, version: StdLibVersion): Either[String, List[DataItem[_]]] =
    c match {
        ...
        fields(FieldNames.Data) match {
          case ARR(xs) =>
            val l: Vector[E[DataItem[_]]] = xs.map {
              ...
              case other => err(other, version, FieldNames.DataEntry)
            }.toVector
            l.sequence.map(_.toList)
          case other => err(other, version, s"List(${FieldNames.Data})")
        }
      case other => err(other, version)
    }
  private def processTransferSet(c: CaseObj, version: StdLibVersion): Either[String, List[(Address, Long, Option[ByteStr])]] =
    c match {
      case CaseObj(_, fields) =>
        fields(FieldNames.Transfers) match {
          case ARR(xs) =>
            val l: Vector[E[(Address, Long, Option[ByteStr])]] = xs.map {
              ...
              case other => err(other, version, FieldNames.TransferSet)
            }.toVector
            l.sequence.map(_.toList)
          case other => err(other, version, s"List(${FieldNames.Transfers})")
        }
      case other => err(other, version)
    }

  private def processContractSet(c: CaseObj, version: StdLibVersion) = c match {
    case CaseObj(_, fields) =>
      val writes = fields(FieldNames.ScriptWriteSet) match {
        case c @ CaseObj(tpe, _) if tpe.name == FieldNames.WriteSet => processWriteSet(c, version)
        case other                                                  => err(other, version, FieldNames.Data)
      }
      val payments = fields(FieldNames.ScriptTransferSet) match {
        case c @ CaseObj(tpe, _) if tpe.name == FieldNames.TransferSet => processTransferSet(c, version)
        case other                                                     => err(other, version, FieldNames.Transfers)
      }
      for {
        w <- writes
        p <- payments
  ...
  def fromObj(e: EVALUATED, version: StdLibVersion): Either[ExecutionError, ScriptResult] =
    e match {
      case c @ CaseObj(tpe, _) =>
        tpe.name match {
          case FieldNames.WriteSet     => processWriteSet(c, version).map(ScriptResult(_, List.empty))
          case FieldNames.TransferSet  => processTransferSet(c, version).map(ScriptResult(List.empty, _))
          case FieldNames.ScriptResult => processContractSet(c, version)
