package com.wavesplatform.lang.v1.evaluator.ctx

import cats.Monad
import cats.implicits._
import com.wavesplatform.lang.v1.compiler.Terms.CaseObj
import com.wavesplatform.lang.v1.compiler.Types.UNIT
import com.wavesplatform.lang.ExecutionError

package object impl {
  def notImplemented[F[_]: Monad, R](funcName: String, args: List[Any]): F[Either[ExecutionError, R]] =
    ExecutionError(s"Can't apply (${args.map(_.getClass.getSimpleName).mkString(", ")}) to '$funcName'")
      .asLeft[R]
      .pure[F]

  lazy val unit: CaseObj = CaseObj(UNIT, Map.empty)

  def callableResultError(expected: AnyRef, actual: AnyRef): ExecutionError =
    ExecutionError(s"CallableFunction needs to return $expected, but got '$actual'")
}
