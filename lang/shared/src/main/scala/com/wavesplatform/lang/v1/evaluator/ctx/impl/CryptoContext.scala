  ...
  private def buildNew(global: BaseGlobal, version: StdLibVersion): CTX[NoContext] = {
    ...
    def toBase58StringF: BaseFunction[NoContext] =
      NativeFunction(
        ...
        case CONST_BYTESTR(bytes) :: Nil => global.base58Encode(bytes.arr).flatMap(CONST_STRING(_, reduceLimit = version >= V4))
    ...
    def toBase64StringF: BaseFunction[NoContext] =
      NativeFunction(
        ...
        case CONST_BYTESTR(bytes) :: Nil => global.base64Encode(bytes.arr).flatMap(CONST_STRING(_, reduceLimit = version >= V4))
    ...
    def toBase16StringF(checkLength: Boolean): BaseFunction[NoContext] = NativeFunction("toBase16String", 10, TOBASE16, STRING, ("bytes", BYTESTR)) {
      case CONST_BYTESTR(bytes) :: Nil => global.base16Encode(bytes.arr, checkLength).flatMap(CONST_STRING(_))
