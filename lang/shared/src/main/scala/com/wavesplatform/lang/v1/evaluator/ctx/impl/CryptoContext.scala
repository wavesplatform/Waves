...
object CryptoContext {
  ...
  private def digestAlgorithmType(v: StdLibVersion) =
    UNION.create(rsaHashAlgs(v), if (v > V3) {
                                    Some("RsaDigestAlgs")
                                  } else {
                                    None
                                  })
  ...
  private[this] val cachedContexts = new util.HashMap[(BaseGlobal, StdLibVersion), CTX[NoContext]]()
  ...
  private def hashFunction(name: String, internalName: Short, cost: Long)(h: Array[Byte] => Array[Byte]): BaseFunction[NoContext] =
  ...
  private def hashLimFunction(lim: Array[Int], name: String, internalName: Short, costs: Int => Int)(
  ...
  def build(global: BaseGlobal, version: StdLibVersion): CTX[NoContext] = cachedContexts.synchronized {
    cachedContexts.computeIfAbsent(
      (global, version), { _ =>
        val keccak256F  = hashFunction("keccak256", KECCAK256, if (version < V4) 10 else 200)(global.keccak256)
        val blake2b256F = hashFunction("blake2b256", BLAKE256, if (version < V4) 10 else 200)(global.blake2b256)
        val sha256F     = hashFunction("sha256", SHA256, if (version < V4) 10 else 200)(global.sha256)
        ...
        val rsaVerifyF: BaseFunction[NoContext] = {
          ...
          NativeFunction(
            ...
            case xs =>
              notImplemented[Id, EVALUATED](s"rsaVerify(digest: DigestAlgorithmType, message: ByteVector, sig: ByteVector, pub: ByteVector)", xs)
          }
        }
        ...
        def toBase16StringF(checkLength: Boolean): BaseFunction[NoContext] =
          NativeFunction("toBase16String", 10, TOBASE16, STRING, ("bytes", BYTESTR)) {
