...
class RSATest extends PropSpec with BeforeAndAfterAll {
  ...
  property("true on correct signature") {
    forAll(keyPairGenerator, messageGenerator) { (keyPair, message) =>
      ...
      algs foreach { alg =>
        ...
        eval(scriptSrc(alg, message, signature, xpub.getEncoded), PureContext.build(V3, useNewPowPrecision = true) |+| CryptoContext.build(Global, V3, true)) shouldBe Right(CONST_BOOLEAN(true))
      }
    }
  }
  ...
  property("RsaDigestAlgs disabled in V4") {
    forAll(keyPairGenerator, messageGenerator) { (keyPair, message) =>
      ...
      algs foreach { alg =>
        ...
        eval(scriptSrc(alg, message, signature, xpub.getEncoded), PureContext.build(V4, useNewPowPrecision = true) |+| CryptoContext.build(Global, V4, true)) should produce(s"Can't find a function '${algToType(alg)}'()")
      }
    }
  }
    ...
    for(lim <- Seq(16, 32, 64, 128)) {
      forAll(keyPairGenerator, sizedMessageGenerator(lim*1024)) { (keyPair, message) =>
        ...
        algs.filter(_ != NONE) foreach { alg =>
          ...
          val ctx: CTX[NoContext] = PureContext.build(V4, useNewPowPrecision = true) |+| CryptoContext.build(Global, V4, true) |+| CTX[NoContext](Seq(), vars, Array.empty[BaseFunction[NoContext]])
  ...
  property("rsaVerify works with max size V3") {
    forAll(keyPairGenerator, sizedMessageGenerator(32*1024)) { (keyPair, message) =>
      ...
      algs.filter(_ != NONE) foreach { alg =>
        ...
        val ctx: CTX[NoContext] = PureContext.build(V3, useNewPowPrecision = true) |+| CryptoContext.build(Global, V3, true) |+| CTX[NoContext](Seq(), vars, Array.empty[BaseFunction[NoContext]])
  ...
  property("false on incorrect signature") {
    forAll(keyPairGenerator, messageGenerator) { (keyPair, message) =>
      ...
      algs foreach { alg =>
        eval(scriptSrc(alg, message, signature, xpub.getEncoded), PureContext.build(V3, useNewPowPrecision = true) |+| CryptoContext.build(Global, V3, true)) shouldBe Right(CONST_BOOLEAN(false))
      }
    }
  }
  ...
  private def eval[T <: EVALUATED](code: String, ctx: CTX[NoContext] = PureContext.build(V4, useNewPowPrecision = true) |+| CryptoContext.build(Global, V4, true)): Either[String, T] = {
