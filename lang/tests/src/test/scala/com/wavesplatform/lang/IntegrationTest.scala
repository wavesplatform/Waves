...
class IntegrationTest extends PropSpec with PropertyChecks with ScriptGen with Matchers with NoShrink with Inside {
  ...
  property("BigInt") {
    evalV5("toBigInt(fraction(9223372036854775807, -2, -4)) == (toBigInt(9223372036854775807) * toBigInt(-2)) / toBigInt(-4)") shouldBe Right(CONST_BOOLEAN(true))
    evalV5(s"""
    ...
    evalV5(s"""-toBigInt(1)""") shouldBe Right(CONST_BIGINT(BigInt(-1)))
    evalV5("toBigInt(base58'2Ana1pUpv2ZbMVkwF5FXapYeBEjdxDatLn7nvJkhgTSXbs59SyZSx866bXirPgj8QQVB57uxHJBG1YFvkRbFj4T').toString()") shouldBe Right(CONST_STRING("52785833603464895924505196455835395749861094195642486808108138863402869537852026544579466671752822414281401856143643660416162921950916138504990605852480").explicitGet())
    evalV5("toBigInt(base58'3ZRuyAbxDY78aZf8nGASFCZKPhJ5LuDAuALLdoAqCqaFuAjqgmZ47WsE11LKt2JuF5Pasqx65bzvjWMzHB2b4vuC').toString()") shouldBe Right(CONST_STRING("6703903964971298549787012499102923063739682910296196688861780721860882015036773488400937149083451713845015929093243025426876941405973284973216824503042047").explicitGet())
    evalV5("toBigInt(base58'3ZRuyAbxDY78aZf8nGASFCZKPhJ5LuDAuALLdoAqCqaFuAjqgmZ47WsE11LKt2JuF5Pasqx65bzvjWMzHB2b4vuCa').toString()") should produce("Too big ByteVector for BigInt (65 > 64 bytes)")
    evalV5("let bin = base58'2Ana1pUpv2ZbMVkwF5FXapYeBEjdxDatLn7nvJkhgTSXbs59SyZSx866bXirPgj8QQVB57uxHJBG1YFvkRbFj4T' ; toBytes(toBigInt(bin)) == bin") shouldBe Right(CONST_BOOLEAN(true))
    evalV5("toBigInt(base58'a' + base58'2Ana1pUpv2ZbMVkwF5FXapYeBEjdxDatLn7nvJkhgTSXbs59SyZSx866bXirPgj8QQVB57uxHJBG1YFvkRbFj4T' + base58'a', base58'a'.size(), 64).toString()") shouldBe Right(CONST_STRING("52785833603464895924505196455835395749861094195642486808108138863402869537852026544579466671752822414281401856143643660416162921950916138504990605852480").explicitGet())
    evalV5("""parseBigIntValue("6703903964971298549787012499102923063739682910296196688861780721860882015036773488400937149083451713845015929093243025426876941405973284973216824503042047")""") shouldBe Right(CONST_BIGINT(BigInt(2).pow(511) - 1))
    evalV5("""parseBigIntValue("6703903964971298549787012499102923063739682910296196688861780721860882015036773488400937149083451713845015929093243025426876941405973284973216824503042048")""") should produce("too big")
    evalV5("""parseBigIntValue("-6703903964971298549787012499102923063739682910296196688861780721860882015036773488400937149083451713845015929093243025426876941405973284973216824503042048")""") shouldBe Right(CONST_BIGINT(-BigInt(2).pow(511)))
    evalV5("""parseBigIntValue("-6703903964971298549787012499102923063739682910296196688861780721860882015036773488400937149083451713845015929093243025426876941405973284973216824503042049")""") should produce("too big")
    evalV5("""parseBigInt("-6703903964971298549787012499102923063739682910296196688861780721860882015036773488400937149083451713845015929093243025426876941405973284973216824503042048")""") shouldBe Right(CONST_BIGINT(-BigInt(2).pow(511)))
    evalV5("""parseBigInt("6703903964971298549787012499102923063739682910296196688861780721860882015036773488400937149083451713845015929093243025426876941405973284973216824503042048")""") shouldBe Right(unit)
    evalV5(s"""fraction(parseBigIntValue("${BigInt(2).pow(511)-1}"), toBigInt(-2), toBigInt(-3))""") shouldBe Right(CONST_BIGINT((BigInt(2).pow(511)-1)*2/3))
    evalV5(s"""fraction(toBigInt(100), toBigInt(2), toBigInt(0))""") shouldBe Left("Fraction: division by zero")
    evalV5(s"""parseBigIntValue("${Long.MaxValue}").toInt()""") shouldBe Right(CONST_LONG(Long.MaxValue))
    evalV5(s"""parseBigIntValue("${Long.MinValue}").toInt()""") shouldBe Right(CONST_LONG(Long.MinValue))
    evalV5(s"""(parseBigIntValue("${Long.MaxValue}")+toBigInt(1)).toInt()""") should produce("out of integers range")
  }
  ...
  property("BigInt fraction rounding") {
    ...
    for {
      ...
      evalV5(s"""fraction(toBigInt(${10*s1}), toBigInt(1), toBigInt(${3*s2}), ${r._1})""") shouldBe
        Right(CONST_BIGINT(BigInt(BigDecimal((10.0*s1)/(3.0*s2)).setScale(0, r._2).toLong)))
      evalV5(s"""fraction(toBigInt(${9*s1}), toBigInt(1), toBigInt(${2*s2}), ${r._1})""") shouldBe
        Right(CONST_BIGINT(BigInt(BigDecimal((9.0*s1)/(2.0*s2)).setScale(0, r._2).toLong)))
      evalV5(s"""fraction(toBigInt(${11*s1}), toBigInt(1), toBigInt(${2*s2}), ${r._1})""") shouldBe
        Right(CONST_BIGINT(BigInt(BigDecimal((11.0*s1)/(2.0*s2)).setScale(0, r._2).toLong)))
      evalV5(s"""fraction(toBigInt(100), toBigInt(2), toBigInt(0), ${r._1})""") shouldBe
        Left("Fraction: division by zero")
    }
  }

  property("BigInt comparison") {
    evalV5("toBigInt(16) > toBigInt(2)") shouldBe Right(CONST_BOOLEAN(true))
    evalV5("toBigInt(1) > toBigInt(2)") shouldBe Right(CONST_BOOLEAN(false))
    evalV5("toBigInt(16) >= toBigInt(2)") shouldBe Right(CONST_BOOLEAN(true))
    evalV5("toBigInt(1) >= toBigInt(2)") shouldBe Right(CONST_BOOLEAN(false))
    evalV5("toBigInt(16) >= toBigInt(16)") shouldBe Right(CONST_BOOLEAN(true))
    evalV5("toBigInt(16) < toBigInt(2)") shouldBe Right(CONST_BOOLEAN(false))
    evalV5("toBigInt(16) <= toBigInt(2)") shouldBe Right(CONST_BOOLEAN(false))
    evalV5("toBigInt(16) <= toBigInt(16)") shouldBe Right(CONST_BOOLEAN(true))
  }

  property("BigInt min and max") {
    evalV5("[toBigInt(16), toBigInt(8)].max()") shouldBe Right(CONST_BIGINT(BigInt(16)))
    evalV5("[toBigInt(16), toBigInt(8)].min()") shouldBe Right(CONST_BIGINT(BigInt(8)))
  }

  property("BigInt math functions") {
    evalV5("pow(toBigInt(2), 0, toBigInt(510), 0, 0, DOWN)") shouldBe Right(CONST_BIGINT(BigInt(2) pow 510))
    evalV5("pow(toBigInt(2), 0, toBigInt(511), 0, 0, DOWN)") should produce("Result out of 512-bit range on BigInt pow calculation")
    evalV5("pow(toBigInt(12), 1, toBigInt(3456), 3, 2, DOWN)") shouldBe Right(CONST_BIGINT(BigInt(187)))
    evalV5("pow(toBigInt(0), 1, toBigInt(3456), 3, 2, DOWN)") shouldBe Right(CONST_BIGINT(BigInt(0)))
    evalV5("pow(toBigInt(20), 1, toBigInt(-1), 0, 4, DOWN)") shouldBe Right(CONST_BIGINT(BigInt(5000)))
    evalV5("pow(toBigInt(-20), 1, toBigInt(-1), 0, 4, DOWN)") shouldBe Right(CONST_BIGINT(BigInt(-5000)))
    evalV5("pow(toBigInt(0), 1, toBigInt(-1), 0, 4, DOWN)") should produce("Division by zero on BigInt pow calculation")
    evalV5("pow(toBigInt(2), 0, toBigInt(512), 0, 0, DOWN)") should produce("Result out of 512-bit range on BigInt pow calculation")
    evalV5("log(toBigInt(16), 0, toBigInt(2), 0, 0, CEILING)") shouldBe Right(CONST_BIGINT(BigInt(4)))
    evalV5("log(toBigInt(1), 4, toBigInt(1), 1, 0, HALFEVEN)") shouldBe Right(CONST_BIGINT(BigInt(4)))
    evalV5("log(toBigInt(16), 0, toBigInt(-2), 0, 0, CEILING)") should produce("Illegal log(x) for x <= 0: x = -2 on BigInt log calculation")
    evalV5("log(toBigInt(-16), 0, toBigInt(2), 0, 0, CEILING)") should produce("Illegal log(x) for x <= 0: x = -16 on BigInt log calculation")
    evalV5("""log(toBigInt(1), 16, parseBigIntValue("10"), 0, 0, CEILING)""") shouldBe Right(CONST_BIGINT(BigInt(-16)))
  }
  ...
  property("List[BigInt] median - 100 elements") {
    ...
    val arrSorted = arr.sorted
    val src =
      s"[toBigInt(${arr.mkString("),toBigInt(")})].median()"
    evalV5(src) shouldBe Right(CONST_BIGINT(BigInt(Math.floorDiv(arrSorted(49) + arrSorted(50), 2))))
  }
  ...
  property("List[BigInt] median - 99 elements") {
    ...
    val src =
      s"[toBigInt(${arr.mkString("),toBigInt(")})].median()"
    evalV5(src) shouldBe Right(CONST_BIGINT(BigInt(arrSorted(49))))
  }

  property("List[BigInt] median - 1 elements") {
    evalV5(s"[toBigInt(0)].median()") shouldBe Right(CONST_BIGINT(BigInt(0)))
    evalV5(s"""[parseBigIntValue("6703903964971298549787012499102923063739682910296196688861780721860882015036773488400937149083451713845015929093243025426876941405973284973216824503042047")].median()""") shouldBe Right(CONST_BIGINT((BigInt(2) pow 511) - 1))
    evalV5(s"""[parseBigIntValue("-6703903964971298549787012499102923063739682910296196688861780721860882015036773488400937149083451713845015929093243025426876941405973284973216824503042048")].median()""") shouldBe Right(CONST_BIGINT(BigInt(-2) pow 511))
  }

  property("List[BigInt] median - 1000 elements - success") {
    val maxValue = s"""parseBigIntValue("${(BigInt(2) pow 511) - 1}")"""
    val minValue = s"""parseBigIntValue("${-BigInt(2) pow 511}")"""
    evalV5(s"""[${(1 to 1000).map(_ => maxValue).mkString(",")}].median()""") shouldBe Right(CONST_BIGINT((BigInt(2) pow 511) - 1))
    evalV5(s"""[${(1 to 1000).map(_ => minValue).mkString(",")}].median()""") shouldBe Right(CONST_BIGINT(-BigInt(2) pow 511))
  }

  property("List[BigInt] median - negative rounding down") {
    evalV5(s"[toBigInt(${Seq(3, -8).mkString("),toBigInt(")})].median()") shouldBe Right(CONST_BIGINT(BigInt(-3)))
  }

  property("List[BigInt] median - empty list - error") {
    evalV5(s"[toBigInt(1)].removeByIndex(0).median()") should produce("Can't find median for empty list of BigInt")
  }
