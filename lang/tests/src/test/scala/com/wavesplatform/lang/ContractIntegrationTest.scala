...
import com.wavesplatform.lang.v1.traits.domain.{AssetTransfer, AttachedPayments, DataItem, Recipient, TransferAttachment, Tx}
...
class ContractIntegrationTest extends PropSpec with PropertyChecks with ScriptGen with Matchers with NoShrink with Inside {
  ...
  property("Simple verify") {
    val dummyTx = Tx.Transfer(
      ...
      attachment = TransferAttachment.bytesExact(ByteStr.empty, V3)
    )
