...
class ScriptEstimatorTestBase(estimators: ScriptEstimator*)
  ...
  protected def estimate(functionCosts: Map[FunctionHeader, Coeval[Long]], script: EXPR): Either[String, Long] = {
    val results = estimators.map(_(ctx.evaluationContext(env).letDefs.keySet, functionCosts, script))
