...
object Terms {
  ...
  object Typed {
    ...
    case class GETTER(ref: BLOCK, field: String, override val tpe: TYPE)              extends EXPR(tpe)
    case class CONST_BYTEVECTOR(bs: ByteVector)                                       extends EXPR(BYTEVECTOR)
    case class SUM(a: BLOCK, b: BLOCK)                                                extends EXPR(INT)
    case class AND(a: BLOCK, b: BLOCK)                                                extends EXPR(BOOLEAN)
    case class OR(a: BLOCK, b: BLOCK)                                                 extends EXPR(BOOLEAN)
    case class EQ(a: BLOCK, b: BLOCK)                                                 extends EXPR(BOOLEAN)
    case class GT(a: BLOCK, b: BLOCK)                                                 extends EXPR(BOOLEAN)
    case class GE(a: BLOCK, b: BLOCK)                                                 extends EXPR(BOOLEAN)
    case class SIG_VERIFY(message: BLOCK, signature: BLOCK, publicKey: BLOCK)         extends EXPR(BOOLEAN)
    case class IS_DEFINED(opt: BLOCK)                                                 extends EXPR(BOOLEAN)
    case class LET(name: String, value: BLOCK)                                        extends EXPR(UNIT)
    case class BLOCK(let: Option[LET], body: EXPR, override val tpe: TYPE)            extends EXPR(tpe)
    case class IF(cond: BLOCK, ifTrue: BLOCK, ifFalse: BLOCK, override val tpe: TYPE) extends EXPR(tpe)
    case class REF(key: String, override val tpe: TYPE)                               extends EXPR(tpe)
    case class GET(opt: BLOCK, override val tpe: TYPE)                                extends EXPR(tpe)
    case object TRUE                                                                  extends EXPR(BOOLEAN)
    case object FALSE                                                                 extends EXPR(BOOLEAN)
    case object NONE                                                                  extends EXPR(OPTION(NOTHING))
    case class SOME(t: BLOCK, override val tpe: TYPE)                                 extends EXPR(tpe)
  }
