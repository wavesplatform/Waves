...
object Preconditions {
  ...
  final case class LeaseP(from: AccountKeyPair, to: Address, amount: Long)                                                       extends PAction(3)
  final case class IssueP(name: String, issuer: AccountKeyPair, desc: String, amount: Long, decimals: Int, reissueable: Boolean) extends PAction(2)
  final case class CreateAccountP(seed: String, balance: Long)                                                                   extends PAction(1)

  final case class PGenSettings(faucet: AccountKeyPair, actions: List[PAction])

  final case class UniverseHolder(accountsWithBalances: List[(AccountKeyPair, Long)] = Nil,
  ...
  private val leasingSectionReader = new ValueReader[LeaseP] {
    override def read(config: Config, path: String): LeaseP = {
      ...
      LeaseP(
        AccountKeyPair.fromSeed(from).explicitGet(),
        AccountKeyPair.fromSeed(to).explicitGet(),
        amount
      )
    }
  }
