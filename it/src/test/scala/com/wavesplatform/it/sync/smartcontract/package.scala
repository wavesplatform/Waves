...
package object smartcontract {
  ...
  def exchangeTx(pair: AssetPair, exTxFee: Long, orderFee: Long, time: Time, ord1Ver: Byte, ord2Ver: Byte, accounts: AccountKeyPair*): JsObject = {
  ...
  def orders(pair: AssetPair, ord1Ver: Byte, ord2Ver: Byte, fee: Long, time: Time, accounts: AccountKeyPair*): (Order, Order) = {
    val buyer               = accounts.head // first one
    val seller              = accounts.tail.head // second one
    val matcher             = accounts.last
