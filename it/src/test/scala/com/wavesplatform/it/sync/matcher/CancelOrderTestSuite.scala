...
class CancelOrderTestSuite extends MatcherSuiteBase {
  ...
  "Order can be canceled" - {
    "by sender" in {
      val orderId = matcherNode.placeOrder(bobAcc, wavesUsdPair, OrderType.SELL, 100.waves, 800, AsyncMatcherHttpApi.DefaultMatcherFee).message.id
    ...
    "works for" - {
      "all orders placed by an address" in {
        ...
        val usdOrderIds = 1 to 5 map { i =>
          matcherNode.placeOrder(bobAcc, wavesUsdPair, OrderType.SELL, 100.waves + i, 400, AsyncMatcherHttpApi.DefaultMatcherFee).message.id
        }

        matcherNode.assetBalance(bobAcc.toAddress.stringRepr, BtcId.base58)

        val btcOrderIds = 1 to 5 map { i =>
          matcherNode.placeOrder(bobAcc, wavesBtcPair, OrderType.BUY, 100.waves + i, 400, AsyncMatcherHttpApi.DefaultMatcherFee).message.id
        }
