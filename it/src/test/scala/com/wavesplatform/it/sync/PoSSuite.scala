...
class PoSSuite extends FunSuite with Matchers with NodesFromDocker with WaitForHeight2 with CancelAfterFailure {
  ...
  val signerPK = AccountKeyPair.fromSeed(nodeConfigs.last.getString("account-seed")).explicitGet()
  ...
  test("Reject block with invalid signature") {
    val otherNodePK = AccountKeyPair.fromSeed(nodeConfigs.head.getString("account-seed")).explicitGet()
  ...
  def forgeBlock(height: Int, signerPK: AccountKeyPair)(updateDelay: Long => Long = identity,
