...
class BlacklistedTradingTestSuite
  ...
  "When blacklists are empty and some orders was placed" - {
    ...
    val ethOrder = matcher.placeOrder(alice, ethWavesPair, SELL, dec8, dec8).message.id
    val btcOrder = matcher.placeOrder(bob, wavesBtcPair, SELL, dec8, dec8).message.id
    ...
    "If some assets and addresses are blacklisted" in {
      ...
      matcher.activeOrderHistory(alice).size shouldBe 3
      matcher.activeOrderHistory(alice).foreach(_.status shouldBe "Accepted")
      matcher.activeOrderHistory(bob).size shouldBe 1
      matcher.activeOrderHistory(bob).head.status shouldBe "Accepted"
      ...
      matcher.reservedBalance(bob).size shouldBe 1
    ...
    "And now if all blacklists are cleared" in {
      ...
      val newWctOrder = matcher.placeOrder(alice, wctWavesPair, BUY, dec2, dec8).message.id
      val newEthOrder = matcher.placeOrder(alice, ethWavesPair, SELL, dec8, dec8).message.id
      val newBtcOrder = matcher.placeOrder(bob, wavesBtcPair, SELL, dec8, dec8).message.id
