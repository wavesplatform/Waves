...
import com.wavesplatform.it.api.{AssetDecimalsInfo, AsyncMatcherHttpApi}
...
class TradeBalanceAndRoundingTestSuite extends MatcherSuiteBase {
  ...
  "Alice and Bob trade WCT-USD sell price less than buy price" - {
    "place wcd-usd order corrected by new price sell amount less then initial one" in {
      ...
      val bobOrderId = matcherNode.placeOrder(bobAcc, wctUsdPair, SELL, sellAmount, sellPrice, AsyncMatcherHttpApi.DefaultMatcherFee).message.id
      matcherNode.waitOrderStatus(wctUsdPair, bobOrderId, "Accepted", 1.minute)
      val aliceOrderId = matcherNode.placeOrder(aliceAcc, wctUsdPair, BUY, buyAmount, buyPrice, AsyncMatcherHttpApi.DefaultMatcherFee).message.id
  ...
  "Alice and Bob trade WCT-USD 1" - {
    ...
    "place wct-usd order" in {
      ...
      val bobOrderId =
        matcherNode.placeOrder(bobAcc, wctUsdPair, SELL, wctUsdSellAmount, wctUsdPrice, AsyncMatcherHttpApi.DefaultMatcherFee).message.id
      matcherNode.waitOrderStatus(wctUsdPair, bobOrderId, "Accepted", 1.minute)

      val aliceOrderId =
        matcherNode.placeOrder(aliceAcc, wctUsdPair, BUY, wctUsdBuyAmount, wctUsdPrice, AsyncMatcherHttpApi.DefaultMatcherFee).message.id
      matcherNode.waitOrderStatus(wctUsdPair, aliceOrderId, "Filled", 1.minute)
    ...
    "reserved balance is empty after the total execution" in {
      val aliceOrderId = matcherNode.placeOrder(aliceAcc, wctUsdPair, BUY, 5000000, 100000, AsyncMatcherHttpApi.DefaultMatcherFee).message.id
      matcherNode.waitOrderStatus(wctUsdPair, aliceOrderId, "Accepted", 1.minute)

      val bobOrderId = matcherNode.placeOrder(bobAcc, wctUsdPair, SELL, 5000000, 99908, AsyncMatcherHttpApi.DefaultMatcherFee).message.id
  ...
  "Alice and Bob trade WCT-WAVES on not enough fee when place order" - {
    ...
    "bob lease all waves exact half matcher fee" in {
      ...
      val bobOrderId =
        matcherNode.placeOrder(bobAcc, wctWavesPair, SELL, wctWavesSellAmount, wctWavesPrice, AsyncMatcherHttpApi.DefaultMatcherFee).message.id
      ...
      assertBadRequestAndResponse(
        matcherNode.placeOrder(bobAcc, wctWavesPair, SELL, wctWavesSellAmount / 2, wctWavesPrice, AsyncMatcherHttpApi.DefaultMatcherFee),
  ...
  "Alice and Bob trade ETH-WAVES" - {
    "reserved balance is empty after the total execution" in {
      val counterId1 = matcherNode.placeOrder(aliceAcc, ethWavesPair, SELL, 2864310, 300000, AsyncMatcherHttpApi.DefaultMatcherFee).message.id
      matcherNode.waitOrderStatus(ethWavesPair, counterId1, "Accepted", 1.minute)

      val counterId2 = matcherNode.placeOrder(aliceAcc, ethWavesPair, SELL, 7237977, 300000, AsyncMatcherHttpApi.DefaultMatcherFee).message.id
      matcherNode.waitOrderStatus(ethWavesPair, counterId2, "Accepted", 1.minute)

      val submittedId = matcherNode.placeOrder(bobAcc, ethWavesPair, BUY, 4373667, 300000, AsyncMatcherHttpApi.DefaultMatcherFee).message.id
