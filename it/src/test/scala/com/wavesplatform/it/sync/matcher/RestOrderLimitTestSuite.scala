...
class RestOrderLimitTestSuite
  ...
  "Order History REST API methods should have limit for orders in response" in {
    ...
    val active0 = matcher.placeOrder(aliceAcc, alicePair, SELL, 1, 15.waves).message.id

    val active1    = matcher.placeOrder(aliceAcc, alicePair, SELL, 1, 10.waves).message.id
    val partial1   = matcher.placeOrder(aliceAcc, alicePair, SELL, 2, 9.waves).message.id
    val filled1    = matcher.placeOrder(aliceAcc, alicePair, SELL, 1, 8.waves).message.id
    val cancelled1 = matcher.placeOrder(aliceAcc, alicePair, SELL, 1, 11.waves).message.id
    val active2    = matcher.placeOrder(aliceAcc, bobPair, BUY, 1, 2.waves).message.id
    val filled2    = matcher.placeOrder(aliceAcc, bobPair, BUY, 1, 4.waves).message.id
    val partial2   = matcher.placeOrder(aliceAcc, bobPair, BUY, 2, 3.waves).message.id
    val cancelled2 = matcher.placeOrder(aliceAcc, bobPair, BUY, 1, 2.waves).message.id

    // orders for matching Alice's orders
    matcher.placeOrder(bobAcc, alicePair, BUY, 1, 8.waves).message.id // fill filled1
    matcher.placeOrder(bobAcc, alicePair, BUY, 1, 9.waves).message.id // part fill partial1
    matcher.placeOrder(bobAcc, bobPair, SELL, 1, 4.waves).message.id  // fill filled2
    matcher.placeOrder(bobAcc, bobPair, SELL, 1, 3.waves).message.id  // part fill partial2
    ...
    val active3 = matcher.placeOrder(aliceAcc, alicePair, SELL, 1, 10.waves).message.id
    val active4 = matcher.placeOrder(aliceAcc, alicePair, SELL, 1, 10.waves).message.id
    val active5 = matcher.placeOrder(aliceAcc, bobPair, BUY, 1, 2.waves).message.id
    val active6 = matcher.placeOrder(aliceAcc, bobPair, BUY, 1, 2.waves).message.id
    ...
    val active7  = matcher.placeOrder(aliceAcc, alicePair, SELL, 1, 9.waves).message.id
    val active8  = matcher.placeOrder(aliceAcc, alicePair, SELL, 1, 10.waves).message.id
    val active9  = matcher.placeOrder(aliceAcc, bobPair, BUY, 1, 1.waves).message.id
    val active10 = matcher.placeOrder(aliceAcc, bobPair, BUY, 1, 1.waves).message.id
    ...
    matcher.placeOrder(bobAcc, bobPair, SELL, 1, 3.waves).message.id   // fill partial2
    matcher.placeOrder(bobAcc, bobPair, SELL, 2, 2.waves).message.id   // fill active2, active5
    matcher.placeOrder(bobAcc, alicePair, BUY, 2, 9.waves).message.id  // fill partial1, active7
    matcher.placeOrder(bobAcc, alicePair, BUY, 1, 10.waves).message.id // fill active1
    ...
    val active11 = matcher.placeOrder(aliceAcc, alicePair, SELL, 1, 10.waves).message.id
    val active12 = matcher.placeOrder(aliceAcc, alicePair, SELL, 1, 10.waves).message.id
    val active13 = matcher.placeOrder(aliceAcc, alicePair, SELL, 1, 10.waves).message.id
    val active14 = matcher.placeOrder(aliceAcc, alicePair, SELL, 1, 10.waves).message.id
    val active15 = matcher.placeOrder(aliceAcc, alicePair, SELL, 1, 10.waves).message.id
