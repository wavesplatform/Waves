  ...
  private def startNodeInternal(nodeConfig: Config): DockerNode =
    try {
      ...
      val configOverrides: String = {
        val ntpServer = Option(System.getenv("NTP_SERVER")).getOrElse("0.ru.pool.ntp.org")

        var config = s"$javaOptions ${renderProperties(asProperties(overrides))} " +
          s"-Dlogback.stdout.level=TRACE -Dlogback.file.level=OFF -Dwaves.network.declared-address=$ip:$networkPort -Dntp.server=$ntpServer "
