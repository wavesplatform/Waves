...
object AsyncMatcherHttpApi extends Assertions {
  ...
  def cancelRequest(sender: AccountKeyPair, orderId: String): CancelOrderRequest = {
  ...
  def batchCancelRequest(sender: AccountKeyPair, timestamp: Long): CancelOrderRequest = {
  ...
  implicit class MatcherAsyncHttpApi(matcherNode: Node) extends NodeAsyncHttpApi(matcherNode) {
    ...
    def cancelOrder(sender: AccountKeyPair, assetPair: AssetPair, orderId: String): Future[MatcherStatusResponse] =
      matcherPost(s"/matcher/orderbook/${assetPair.toUri}/cancel", cancelRequest(sender, orderId)).as[MatcherStatusResponse]

    def expectCancelRejected(sender: AccountKeyPair, assetPair: AssetPair, orderId: String): Future[Unit] = {
    ...
    def cancelOrdersForPair(sender: AccountKeyPair, assetPair: AssetPair, timestamp: Long): Future[MatcherStatusResponse] =
      matcherPost(s"/matcher/orderbook/${assetPair.toUri}/cancel", Json.toJson(batchCancelRequest(sender, timestamp)))
        .as[MatcherStatusResponse]

    def cancelAllOrders(sender: AccountKeyPair, timestamp: Long): Future[MatcherStatusResponse] =
    ...
    def fullOrdersHistory(sender: AccountKeyPair, activeOnly: Option[Boolean] = None): Future[Seq[OrderbookHistory]] =
      activeOnly match {
        case None =>
    ...
    def orderHistoryByPair(sender: AccountKeyPair, assetPair: AssetPair, activeOnly: Boolean = false): Future[Seq[OrderbookHistory]] = {
    ...
    def reservedBalance(sender: AccountKeyPair): Future[Map[String, Long]] =
      matcherGetWithSignature(s"/matcher/balance/reserved/${Base58.encode(sender.publicKey)}", sender).as[Map[String, Long]]

    def tradableBalance(sender: AccountKeyPair, assetPair: AssetPair): Future[Map[String, Long]] =
    ...
    def prepareOrder(sender: AccountKeyPair,
                     pair: AssetPair,
                     orderType: OrderType,
                     amount: Long,
                     price: Long,
    ...
    def placeOrder(sender: AccountKeyPair,
                   pair: AssetPair,
                   orderType: OrderType,
                   amount: Long,
                   price: Long,
    ...
    def ordersByAddress(sender: AccountKeyPair, activeOnly: Boolean): Future[Seq[OrderbookHistory]] =
    ...
    def matcherState(assetPairs: Seq[AssetPair], orders: IndexedSeq[Order], accounts: Seq[AccountKeyPair]): Future[MatcherState] =
      for {
  ...
  private implicit val AccountKeyPairOrd: Ordering[AccountKeyPair] = Ordering.by[AccountKeyPair, String](_.stringRepr)
}
