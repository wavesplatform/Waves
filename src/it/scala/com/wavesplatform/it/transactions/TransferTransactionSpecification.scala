...
class TransferTransactionSpecification(override val allNodes: Seq[Node]) extends IntegrationSuiteWithThreeAddresses {
  ...
  test("can not make transfer without having enough of fee") {
    val f = for {
      ...
      transferFailureAssertion <- assertRequestError(sender.transfer(secondAddress, firstAddress, 104 waves, fee = 2 waves))
  ...
  test("can not make transfer without having enough of waves") {
    val f = for {
      ...
      transferFailureAssertion <- assertRequestError(sender.transfer(secondAddress, firstAddress, 106 waves, fee = 1 waves))
  ...
  test("can not make transfer without having enough of effective balance") {
    val f = for {
      ...
      transferFailureAssertion <- assertRequestError(sender.transfer(firstAddress, secondAddress, 64 waves, fee = 1 waves))
  ...
  test("can not make transfer without having enough of your own waves") {
    val f = for {
      ...
      transferFailureAssertion <- assertRequestError(sender.transfer(secondAddress, firstAddress, 109 waves, fee = 1 waves))
