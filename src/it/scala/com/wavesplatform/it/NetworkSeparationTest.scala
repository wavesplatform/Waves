...
class NetworkSeparationTest(allNodes: Seq[Node], docker: Docker) extends FreeSpec with ScalaFutures with IntegrationPatience
  with Matchers {
    ...
    val targetBlocks = result(for {
      ...
      _ <- {
        traverse(allNodes)(_.waitForHeight(height + 10)).map(_ =>
          allNodes.foreach(docker.disconnectFromNetwork)
        )
      }
      _ <- {
        traverse(allNodes)(_.waitForHeight(height + 20)).map(_ =>
          allNodes.foreach(docker.connectToNetwork))
      }
