...
class ScriptCacheTest extends FreeSpec with Matchers with WithDB with TransactionGen {
  ...
  def blockGen(scripts: List[Script], t: Time): Gen[(Seq[AccountKeyPair], Seq[Block])] = {
  ...
  def withBlockchain(gen: Time => Gen[(Seq[AccountKeyPair], Seq[Block])])(f: (Seq[AccountKeyPair], BlockchainUpdater with NG) => Unit): Unit = {
