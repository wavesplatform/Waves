...
class ConsensusRouteSpec extends RouteSpec("/consensus") with RestAPISettingsHelper with WithDB with PropertyChecks with MockFactory with BlockGen with HistoryTest {
  private val state = mock[SnapshotStateReader]

  private val history = HistoryWriterImpl(db, new ReentrantReadWriteLock(), TestFunctionalitySettings.Stub,
