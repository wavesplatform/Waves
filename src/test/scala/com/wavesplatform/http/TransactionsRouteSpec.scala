...
class TransactionsRouteSpec
  ...
  routePath("/address/{address}/limit/{limit}") - {
    ...
    "returns 400 BadRequest in case" - {
      def assertBadRequest(p: String) = forAll(accountGen) { a =>
      ...
      "of a missing limit" in {
        forAll(accountGen) { a => assertBadRequest(s"/address/${a.address}") }
      }
      ...
      "limit could not be parsed as int" in {
        forAll(accountGen) { a => assertBadRequest(s"/address/${a.address}/qwe") }
      }
    }
    ...
    // @todo remove mock case
    "returns limit in case everything is correct" in {
      forAll(accountGen) { a =>
        Get(routePath(s"/address/${a.address}/limit/10")) ~> route ~> check {
          status shouldEqual StatusCodes.OK
    ...
    // @todo make sure real test passes
