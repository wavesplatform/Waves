...
class BlockchainUpdaterBlockOnlyTest extends PropSpec
  ...
  property("can apply valid blocks") {
    scenario(preconditionsAndPayments(1), db) { case (domain, (genesis, payments)) =>
  ...
  property("can apply, rollback and reprocess valid blocks") {
    scenario(preconditionsAndPayments(2), db) { case (domain, (genesis, payments)) =>
  ...
  property("can't apply block with invalid signature") {
    scenario(preconditionsAndPayments(1), db) { case (domain, (genesis, payment)) =>
  ...
  property("can process 11 blocks and than rollback to genesis") {
    scenario(preconditionsAndPayments(10), db) { case (domain, (genesis, payments)) =>
