...
class BlockchainUpdaterBurnTest extends PropSpec with PropertyChecks with DomainScenarioDrivenPropertyCheck with Matchers with TransactionGen {
  ...
  val preconditions: Gen[Setup] = for {
    ...
    masterToAlice: TransferTransactionV1 = TransferTransactionV1
      .selfSigned(AssetId.Waves, master, alice, 3 * Waves, ts + 1, AssetId.Waves, transferAssetWavesFee, Array.emptyByteArray)
      .explicitGet()
    ...
    burn: BurnTransactionV1       = BurnTransactionV1.selfSigned(alice, Asset(issue.assetId()), quantity / 2, Waves, ts + 200).explicitGet()
    reissue: ReissueTransactionV1 = ReissueTransactionV1.selfSigned(alice, Asset(issue.assetId()), burn.quantity, true, Waves, ts + 300).explicitGet()
  ...
  property("issue -> burn -> reissue in sequential blocks works correctly") {
    scenario(preconditions, localWavesSettings) {
      case (domain, (ts, genesis, masterToAlice, issue, burn, reissue)) =>
        ...
        val assetDescription1 = domain.blockchainUpdater.assetDescription(Asset(issue.assetId())).get
        ...
        val assetDescription2 = domain.blockchainUpdater.assetDescription(Asset(issue.assetId())).get
