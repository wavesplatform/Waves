...
class BlockchainUpdaterMicroblockSunnyDayTest extends PropSpec
  ...
  property("all txs in different blocks: B0 <- B1 <- B2 <- B3!") {
    scenario(preconditionsAndPayments, db) { case (domain, (genesis, masterToAlice, aliceToBob, aliceToBob2)) =>
  ...
  property("all txs in one block: B0 <- B0m1 <- B0m2 <- B0m3!") {
    scenario(preconditionsAndPayments, db, MicroblocksActivatedAt0WavesSettings) { case (domain, (genesis, masterToAlice, aliceToBob, aliceToBob2)) =>
  ...
  property("discarding some of microBlocks doesn't affect resulting state") {
    forAll(preconditionsAndPayments, accountGen) { case ((genesis, masterToAlice, aliceToBob, aliceToBob2), miner) =>
      ...
      val domainA = domain(db, MicroblocksActivatedAt0WavesSettings)
      ...
      domainA.blockchainUpdater.processBlock(block0a).explicitGet()
      domainA.blockchainUpdater.processBlock(block1a).explicitGet()
      domainA.blockchainUpdater.processMicroBlock(microBlocks1a(0)).explicitGet()
      domainA.blockchainUpdater.processBlock(block2a).explicitGet()
      domainA.blockchainUpdater.processBlock(block3a).explicitGet()


      val domainB = domain(db, MicroblocksActivatedAt0WavesSettings)
      ...
      domainB.blockchainUpdater.processBlock(block0b).explicitGet()
      domainB.blockchainUpdater.processBlock(block1b).explicitGet()
      domainB.blockchainUpdater.processBlock(block2b).explicitGet()
      domainB.blockchainUpdater.processBlock(block3b).explicitGet()

      domainA.stateReader().partialPortfolio(miner).balance shouldBe domainB.stateReader().partialPortfolio(miner).balance
    }
  }
