...
class BlockchainUpdaterMicroblockBadSignaturesTest extends PropSpec with PropertyChecks
  with DomainScenarioDrivenPropertyCheck with Matchers with TransactionGen with WithDB {
  ...
  property("bad total resulting block signature") {
    scenario(preconditionsAndPayments, db) { case (domain, (genesis, payment, payment2)) =>
