  ...
  val preconditions: Gen[(GenesisTransaction, IssueTransactionV2, SetAssetScriptTransaction, TransferTransactionV1)] =
    for {
      ...
      emptyExprScript = ExprScript(
        V3,
        ExpressionCompiler(compilerContext(V3, ContentType.Expression, isAssetScript = true), parsedEmptyScript).explicitGet()._1)
        .explicitGet()
      ...
      typedScript = ExprScript(V3,
                               ExpressionCompiler(compilerContext(V3, ContentType.Expression, isAssetScript = true), untypedScript).explicitGet()._1)
        .explicitGet()
