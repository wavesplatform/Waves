...
trait ScriptGen {
  ...
  private def CONST_INTgen: Gen[CONST_INT] = Gen.choose(Int.MinValue, Int.MaxValue).map(CONST_INT)

  private def HEIGHTgen: Gen[Terms.HEIGHT.type] = Gen.const(HEIGHT)

  private def SUMgen(gas: Int): Gen[SUM] = for {
    i1 <- INTGen((gas - 2) / 2)
    i2 <- INTGen((gas - 2) / 2)
  } yield SUM(i1, i2)

  private def INTGen(gas: Int): Gen[INT] = if (gas > 0) Gen.oneOf(HEIGHTgen, CONST_INTgen, SUMgen(gas - 1), IF_INTgen(gas - 1)) else Gen.oneOf(HEIGHTgen, CONST_INTgen)

  private def GEgen(gas: Int): Gen[GE] = for {
    i1 <- INTGen((gas - 2) / 2)
    i2 <- INTGen((gas - 2) / 2)
  } yield GE(i1, i2)
  ...
  private def IF_BOOLgen(gas: Int): Gen[BOOL] = for {
    cnd <- BOOLgen((gas - 3) / 3)
    t <- BOOLgen((gas - 3) / 3)
    f <- BOOLgen((gas - 3) / 3)
  } yield IF(cnd, t, f)

  private def IF_INTgen(gas: Int): Gen[INT] = for {
    cnd <- BOOLgen((gas - 3) / 3)
    t <- INTGen((gas - 3) / 3)
    f <- INTGen((gas - 3) / 3)
  } yield IF(cnd, t, f)
