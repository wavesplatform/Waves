...
class CommonValidationTimeTest extends PropSpec with PropertyChecks with GeneratorDrivenPropertyChecks with Matchers with TransactionGen {
  ...
  property("disallows too old transacions") {
    forAll(for {
      ...
      TransactionDiffer(TestFunctionality.EnabledFunctionalities, Some(prevBlockTs), blockTs, height)(newState(), transfer1) should produce("too old")
    }
  }
  ...
  property("disallows transactions from far future") {
    forAll(for {
      ...
      val functionalitySettings = TestFunctionality.EnabledSettings.copy(allowTransactionsFromFutureUntil = blockTs - 1)
      val fn = new Functionalities(functionalitySettings, TestFunctionality.EnabledProvider)
      TransactionDiffer(fn, Some(prevBlockTs), blockTs, height)(newState(), transfer1) should produce("far future")
    }
  }
}
