...
class BlockchainSettingsSpecification extends FlatSpec with Matchers {
  "BlockchainSettings" should "read custom values" in {
    ...
    settings.functionalitySettings.allowMakeAssetNameUniqueTransactionAfterTimestamp should be(16)
    settings.functionalitySettings.allowTransferLeasedBalanceUntil should be(17)
  ...
  it should "read testnet settings" in {
    ...
    settings.functionalitySettings.allowTransferLeasedBalanceUntil should be(1495238400000L)
    settings.genesisSettings.blockTimestamp should be(1460678400000L)
