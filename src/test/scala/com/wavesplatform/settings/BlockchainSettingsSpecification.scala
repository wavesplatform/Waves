...
class BlockchainSettingsSpecification extends FlatSpec with Matchers {
  "BlockchainSettings" should "read custom values" in {
    val config = ConfigFactory.parseString(
      """
        ...
        |        allow-leased-balance-transfer-until: 15
        |      }
        |      genesis {
    ...
    settings.functionalitySettings.allowLeasedBalanceTransferUntil should be(15)
  ...
  it should "read testnet settings" in {
    ...
    settings.functionalitySettings.allowLeasedBalanceTransferUntil should be(1495497600000L)
