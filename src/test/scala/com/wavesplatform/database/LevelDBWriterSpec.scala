...
class LevelDBWriterSpec extends FreeSpec with Matchers with TransactionGen with WithDB with DBCacheSettings with RequestGen {
  ...
  "correctly reassemble block from header and transactions" in {
    ...
    try {
      ...
      bcu.blockAtBytes(1).get shouldBe genesisBlock.bytes()
      bcu.blockAtBytes(2).get shouldBe block1.bytes()
      bcu.blockAtBytes(3).get shouldBe block2.bytes()
      bcu.blockAtBytes(4).get shouldBe block3.bytes()
