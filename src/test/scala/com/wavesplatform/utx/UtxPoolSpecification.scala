...
class UtxPoolSpecification
  ...
  private def transfer(sender: AccountKeyPair, maxAmount: Long, time: Time) =
    (for {
  ...
  private def preconditionsGen(lastBlockId: ByteStr, master: AccountKeyPair): Gen[Seq[Block]] =
    for {
  ...
  private def withScriptedAccount(scEnabled: Boolean): Gen[(AccountKeyPair, Long, UtxPoolImpl, Long)] =
    for {
      (sender, senderBalance, bcu) <- stateGen
  ...
  private def transactionV1Gen(sender: AccountKeyPair, ts: Long, feeAmount: Long): Gen[TransferTransactionV1] = accountGen.map { recipient =>
    TransferTransactionV1.selfSigned(Waves, sender, recipient, waves(1), ts, Waves, feeAmount, Array.emptyByteArray).explicitGet()
  }

  private def transactionV2Gen(sender: AccountKeyPair, ts: Long, feeAmount: Long): Gen[TransferTransactionV2] = accountGen.map { recipient =>
  ...
  "UTX Pool" - {
    ...
    "correctly process constrainst in packUnconfirmed" in {
      withDomain(wavesplatform.history.TransfersV2ActivatedAt0WavesSettings) { d =>
        val generateBlock: Gen[(AccountKeyPair, Block, Seq[Transaction], Seq[Transaction])] =
          for {
