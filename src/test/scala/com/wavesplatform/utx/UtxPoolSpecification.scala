...
class UtxPoolSpecification extends FreeSpec with Matchers with MockFactory with PropertyChecks with TransactionGen with NoShrink {
  ...
  private val twoOutOfManyValidPayments = (for {
    ...
    val utx =
      new UtxPoolImpl(
        ...
        UtxSettings(10, PoolDefaultMaxBytes, Set.empty, Set.empty, 5.minutes, 1.minute, allowTransactionsFromSmartAccounts = true)
      )
  ...
  private val withValidPayments = (for {
    ...
    val settings = UtxSettings(10, PoolDefaultMaxBytes, Set.empty, Set.empty, 5.minutes, 1.minute, allowTransactionsFromSmartAccounts = true)
  ...
  private val withBlacklisted = (for {
    ...
    val settings =
      UtxSettings(10, PoolDefaultMaxBytes, Set(sender.address), Set.empty, 5.minutes, 1.minute, allowTransactionsFromSmartAccounts = true)
    val utxPool = new UtxPoolImpl(time, bcu, ignoreSpendableBalanceChanged, FunctionalitySettings.TESTNET, settings)
    (sender, utxPool, txs)
  ...
  private val withBlacklistedAndAllowedByRule = (for {
    ...
    val settings =
      UtxSettings(txs.length, PoolDefaultMaxBytes, Set(sender.address), Set(recipient.address), 5.minutes, 1.minute, allowTransactionsFromSmartAccounts = true)
    val utxPool = new UtxPoolImpl(time, bcu, ignoreSpendableBalanceChanged, FunctionalitySettings.TESTNET, settings)
    (sender, utxPool, txs)
  ...
  private def massTransferWithBlacklisted(allowRecipients: Boolean) =
    (for {
      ...
      val settings =
        UtxSettings(txs.length, PoolDefaultMaxBytes, Set(sender.address), whitelist, 5.minutes, 1.minute, allowTransactionsFromSmartAccounts = true)
      val utxPool = new UtxPoolImpl(time, bcu, ignoreSpendableBalanceChanged, FunctionalitySettings.TESTNET, settings)
      (sender, utxPool, txs)
    }).label("massTransferWithBlacklisted")

  private def utxTest(utxSettings: UtxSettings =
                        UtxSettings(20, PoolDefaultMaxBytes, Set.empty, Set.empty, 5.minutes, 1.minute, allowTransactionsFromSmartAccounts = true),
  ...
  private def withScriptedAccount(scEnabled: Boolean): Gen[(PrivateKeyAccount, Long, UtxPoolImpl, Long)] =
    for {
      ...
      val utx = new UtxPoolImpl(
        ...
        ignoreSpendableBalanceChanged,
        smartAccountsFs,
        UtxSettings(10, PoolDefaultMaxBytes, Set.empty, Set.empty, 1.day, 1.hour, allowTransactionsFromSmartAccounts = scEnabled)
      )
  ...
  "UTX Pool" - {
    "does not add new transactions when full" in utxTest(
      UtxSettings(1, PoolDefaultMaxBytes, Set.empty, Set.empty, 5.minutes, 1.minute, allowTransactionsFromSmartAccounts = true)) { (txs, utx, _) =>
    ...
    "does not add new transactions when full in bytes" in utxTest(
      UtxSettings(999999, 152, Set.empty, Set.empty, 5.minutes, 1.minute, allowTransactionsFromSmartAccounts = true)) { (txs, utx, _) =>
      utx.putIfNew(txs.head) shouldBe 'right
