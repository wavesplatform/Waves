  ...
  property("VersionedTransferTransactionSpecification id doesn't depend on proof") {
    forAll(versionGen, accountGen, accountGen, proofsGen, proofsGen, bytes32gen) {
      case (version, acc1, acc2, proofs1, proofs2, attachment) =>
        val tx1 = VersionedTransferTransaction.create(version, None, acc2, acc2.toAddress, 1, 1, None, 1, attachment, proofs1).explicitGet()
        val tx2 = VersionedTransferTransaction.create(version, None, acc2, acc2.toAddress, 1, 1, None, 1, attachment, proofs2).explicitGet()
