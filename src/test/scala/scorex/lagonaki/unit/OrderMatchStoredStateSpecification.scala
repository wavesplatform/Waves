...
class OrderMatchStoredStateSpecification extends FunSuite with Matchers with BeforeAndAfterAll with BeforeAndAfterEach {
  ...
  lazy val stateFile = Files.createTempFile(Paths.get("/tmp/scorex/"),  "state", "dat")
  ...
  test("Validate enough order balances") {
    ...
    val buy1 = Order.buy(buyAcc, matcher, pair, price, 10*Order.PriceConstant, getTimestamp + Order.MaxLiveTime, 1*WAVES_UNITS)
    val sell1 = Order.sell(sellAcc, matcher, pair, price, 5*Order.PriceConstant, getTimestamp + Order.MaxLiveTime, 1*WAVES_UNITS)
    ...
    om1buy1 should be (5*Order.PriceConstant)
    ...
    val sell2 = Order.sell(sellAcc, matcher, pair, price, 6*Order.PriceConstant, getTimestamp + Order.MaxLiveTime, 1*WAVES_UNITS)
    val notEnoughRemainingOm = createOrderMatch(buy1, sell2, price, 6*Order.PriceConstant, buy1Fee, sell1.matcherFee, matcherTxFee)
    ...
    val sell3 = Order.sell(sellAcc, matcher, pair, price, om1sell1 + 1, getTimestamp + Order.MaxLiveTime, 1*WAVES_UNITS)
    ...
    val sell4 = Order.sell(sellAcc, matcher, pair, price, 5*Order.PriceConstant, getTimestamp + Order.MaxLiveTime, 1*WAVES_UNITS)
    ...
    om2buy1 should be (buy1.amount)
    om2buy2 should be (initBuy2 - BigInt(buy1.amount)*Order.PriceConstant/price)
  ...
  test("Validate not enough balances for fee") {
    ...
    val buy = Order.buy(buyAcc, matcher, pair, price, 5*Order.PriceConstant, getTimestamp + Order.MaxLiveTime, 1*WAVES_UNITS)
    val sell = Order.sell(sellAcc, matcher, pair, price, 10*Order.PriceConstant, getTimestamp + Order.MaxLiveTime, 1*WAVES_UNITS)
