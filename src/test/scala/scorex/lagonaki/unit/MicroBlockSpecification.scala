...
class MicroBlockSpecification extends FunSuite with Matchers with MockFactory with ShouldVerb {
  ...
  test("MicroBlock with txs bytes/parse roundtrip") {
    ...
    val tr: V1TransferTransaction  = V1TransferTransaction.create(None, sender, gen, 5, ts + 1, None, 2, Array()).right.get
    val assetId                    = Some(ByteStr(Array.fill(AssetIdLength)(Random.nextInt(100).toByte)))
    val tr2: V1TransferTransaction = V1TransferTransaction.create(assetId, sender, gen, 5, ts + 2, None, 2, Array()).right.get
  ...
  test("MicroBlock cannot be created with zero transactions") {
    ...
    val transactions       = Seq.empty[V1TransferTransaction]
  ...
  test("MicroBlock cannot contain more than Miner.MaxTransactionsPerMicroblock") {
    ...
    val transaction  = V1TransferTransaction.create(None, sender, gen, 5, System.currentTimeMillis(), None, 1000, Array()).right.get
