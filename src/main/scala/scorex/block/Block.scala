...
object Block extends ScorexLogging {
  ...
  def transParseBytes(version: Int, bytes: Array[Byte]): Try[Seq[Transaction]] = Try {
    if (bytes.isEmpty) {
      ...
      val v: (Array[Byte], Int) = version match {
        case 1 => (bytes.tail, bytes.head) // 255 max
        case 2 => (bytes.tail.tail, bytes.head * 256 + bytes.tail.head) // 65535 max
        case x => ???
      }
  ...
  def genesis(consensusGenesisData: NxtLikeConsensusBlockData,
    ...
    val transactionGenesisDataField = TransactionsBlockFieldVersion1(transactionGenesisData)
