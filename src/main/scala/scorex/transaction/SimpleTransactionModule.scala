...
class SimpleTransactionModule(hardForkParams: ChainParameters)(implicit val settings: WavesSettings,
  ...
  override def createPayment(payment: PaymentRequest, wallet: Wallet): Either[ValidationError, PaymentTransaction] = {
    createPayment(wallet.privateKeyAccount(payment.sender).get, Account.fromBase58String(payment.recipient), payment.amount, payment.fee)
  }
    ...
    findPrivateKey(request.sender)(wallet).flatMap(senderPrivateKey =>
      TransferTransaction
        .create(request.assetId.map(s => Base58.decode(s).get),
          senderPrivateKey,
          Account.fromBase58String(request.recipient),
          request.amount,
          getTimestamp,
  ...
  def lease(request: LeaseRequest, wallet: Wallet): Either[ValidationError, LeaseTransaction] = {
    findPrivateKey(request.sender)(wallet).flatMap(senderPrivateKey =>
      ...
      LeaseTransaction.create(senderPrivateKey, request.amount, request.fee, getTimestamp, Account.fromBase58String(request.recipient))
      .flatMap(onNewOffchainTransaction))
  }
  ...
  override def broadcastPayment(payment: SignedPaymentRequest): Either[ValidationError, PaymentTransaction] =
    for {
      ...
      _t <- PaymentTransaction.create(_sender, Account.fromBase58String(payment.recipient), payment.amount, payment.fee, payment.timestamp, _signature)
