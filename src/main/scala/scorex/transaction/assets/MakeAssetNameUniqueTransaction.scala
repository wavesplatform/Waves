...
object MakeAssetNameUniqueTransaction {
  ...
  def parseTail(bytes: Array[Byte]): Try[MakeAssetNameUniqueTransaction] = Try {
    ...
    val assetId       = EqByteArray(bytes.slice(1 + KeyLength, 1 + KeyLength + AssetIdLength))
    ...
    val signature = EqByteArray(bytes.slice(quantityStart + 16, quantityStart + 16 + SignatureLength))
  ...
  private def createUnverified(sender: PublicKeyAccount,
                               ...
                               timestamp: Long,
                               networkByte: Byte,
                               signature: Option[ByteArray] = None): Either[ValidationError, MakeAssetNameUniqueTransactionImpl] =
    if (fee <= 0) {
  ...
  def create(sender: PublicKeyAccount,
             ...
             timestamp: Long,
             networkByte: Byte,
             signature: ByteArray): Either[ValidationError, MakeAssetNameUniqueTransaction] =
