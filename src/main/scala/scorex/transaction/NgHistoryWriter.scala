      ...
      case Some(base) =>
        micros().headOption match {
          case None if base.uniqueId != microBlock.prevResBlockSig =>
            Left(MicroBlockAppendError("it can't be appended because it is first micro and it doesn't reference existing base block", microBlock))
          case Some(prevMicro) if prevMicro.totalResBlockSig != microBlock.prevResBlockSig =>
            Left(MicroBlockAppendError("it doesn't reference last known microBlock", microBlock))
          case _ =>
