...
class GenesisTransaction(recipient:Account, amount:BigDecimal, timestamp:Long)
	...
	override def toBytes() = {
		...
		Bytes.concat(typeBytes, timestampBytes, amountBytes,
			Base58.decode(this.recipient.getAddress()), Bytes.concat(amountFill, amountBytes))
	}
	...
	override def isValid(db:DBSet) =
		...
		}else if(!Crypto.isValidAddress(this.recipient.getAddress)) {
	...
	override def process(db:DBSet){
		...
		//UPDATE BALANCE
		recipient.setConfirmedBalance(this.amount, db)
