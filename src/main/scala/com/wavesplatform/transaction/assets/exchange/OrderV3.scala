...
case class OrderV3(senderPublicKey: PublicKeyAccount,
                   ...
                   expiration: Long,
                   matcherFee: Long,
                   override val matcherFeeAssetId: AssetId,
                   proofs: Proofs)
    extends Order {
...
object OrderV3 {
  ...
  def parseBytes(bytes: Array[Byte]): Try[Order] = Try {
    ...
    val makeOrder = for {
      ...
      amountAssetId     <- parse(Deser.parseByteArrayOption, AssetIdLength).map(arrOpt => AssetId.fromCompatId(arrOpt.map(ByteStr(_))))
      priceAssetId      <- parse(Deser.parseByteArrayOption, AssetIdLength).map(arrOpt => AssetId.fromCompatId(arrOpt.map(ByteStr(_))))
      orderType         <- readByte
      ...
      matcherFeeAssetId <- parse(Deser.parseByteArrayOption, AssetIdLength).map(arrOpt => AssetId.fromCompatId(arrOpt.map(ByteStr(_))))
