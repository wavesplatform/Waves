...
object AssetId {
  ...
  implicit def fromByteString(bytes: ByteString): AssetId                                                = new AssetId(bytes.toByteArray)
  implicit def toByteString(assetId: AssetId): ByteString                                                = ByteString.copyFrom(assetId.bytes)
