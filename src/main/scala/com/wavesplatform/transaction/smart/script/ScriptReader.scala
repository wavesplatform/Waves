...
object ScriptReader {
  ...
  def fromBytes(bytes: Array[Byte], checks: Boolean = true): Either[ScriptParseError, Script] = {
      ...
      s <- scriptType match {
        case ContentType.Expression =>
          for {
            ...
            s     <- ExprScript(stdLibVersion, bytes, checkSize = false, checkComplexity = checks)
          } yield s
