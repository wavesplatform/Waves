...
object RxExtensionLoader extends ScorexLogging {
  ...
  def apply(maxRollback: Int, syncTimeOut: FiniteDuration,
      ...
      cc match {
        case ChannelClosedAndSyncWith(_, None) =>
          state.loaderState match {
            case wp: LoaderState.WithPeer => state.withIdleLoader
            case _ => state
          }
    ...
    Observable.merge(
      signatures.observeOn(scheduler).mapTask { case ((ch, sigs)) => Task(onNewSignatures(s, ch, sigs)) },
      blocks.observeOn(scheduler).mapTask { case ((ch, block)) => Task(onBlock(s, ch, block)) },
      syncWithChannelClosed.observeOn(scheduler).mapTask(ch => Task(onNewSyncWithChannelClosed(s, ch))))
      .map { ns =>
        s = ns
