...
object Keys {
  ...
  def assetList(addressId: BigInt): Key[Set[Asset]] =
    Key("asset-list", addr(7, addressId), readTxIds(_).toSet.map(Asset), assets => writeTxIds(assets.toSeq.map(_.id)))
  def assetBalanceHistory(addressId: BigInt, asset: Asset): Key[Seq[Int]] =
    historyKey("asset-balance-history", 8, addressId.toByteArray ++ asset.id.arr)
  def assetBalance(addressId: BigInt, asset: Asset)(height: Int): Key[Long] =
  ...
  def assetInfoHistory(asset: Asset): Key[Seq[Int]] = historyKey("asset-info-history", 10, asset.id.arr)
  def assetInfo(asset: Asset)(height: Int): Key[AssetInfo] =
  ...
  def sponsorshipHistory(asset: Asset): Key[Seq[Int]] = historyKey("sponsorship-history", 35, asset.id.arr)
  def sponsorship(asset: Asset)(height: Int): Key[SponsorshipValue] =
  ...
  def addressesForAssetSeqNr(asset: Asset): Key[Int] = bytesSeqNr("addresses-for-asset-seq-nr", 39, asset.id.arr)
  def addressesForAsset(asset: Asset, seqNr: Int): Key[Seq[BigInt]] =
  ...
  def assetScriptHistory(asset: Asset): Key[Seq[Int]] = historyKey("asset-script-history", 46, asset.id.arr)
  def assetScript(asset: Asset)(height: Int): Key[Option[Script]] =
    Key.opt("asset-script", hBytes(47, height, asset.id.arr), ScriptReader.fromBytes(_).explicitGet(), _.bytes().arr)
  def assetScriptPresent(asset: Asset)(height: Int): Key[Option[Unit]] =
