...
object BlockchainFeatures {
  ...
  val FairPOS                         = BlockchainFeature(7, "Fair POS")

  private val dict = Seq(SmallerMinimalGeneratingBalance, NG, MassTransfer, /*SmartAccounts, */ DataTransaction, BurnAnyTokens, FairPOS)
    .map(f => f.id -> f)
    .toMap
