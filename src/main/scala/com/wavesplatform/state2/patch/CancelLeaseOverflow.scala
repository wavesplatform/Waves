...
object CancelLeaseOverflow extends ScorexLogging {
  def apply(s: SnapshotStateReader): Diff = {
    ...
    log.info(
      s"Finished cancelling all lease overflows for sender. Transactions: ${leasesToCancel.mkString(", ")}\n${addressesWithLeaseOverflow.mkString("\n")}")

    Diff.empty.copy(portfolios = addressesWithLeaseOverflow.mapValues(p => Portfolio(0, LeaseBalance(0, -p.lease.out), Map.empty)),
