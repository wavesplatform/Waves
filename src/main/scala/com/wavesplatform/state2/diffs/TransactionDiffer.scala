...
object TransactionDiffer {
  ...
  def apply(settings: FunctionalitySettings, prevBlockTimestamp: Option[Long], currentBlockTimestamp: Long, currentBlockHeight: Int)(
    ...
    for {
      ...
      _ <- CommonValidation.checkTxFee(tx, s, currentBlockHeight, fp, settings)
      diff <- tx match {
        ...
        case ttx: TransferTransaction           => TransferTransactionDiff(s, fp, settings, currentBlockTimestamp, currentBlockHeight)(ttx)
        ...
        case stx: SponsorFeeTransaction         => AssetTransactionsDiff.sponsor(s, settings, currentBlockTimestamp, currentBlockHeight)(stx)
        case ctx: CancelFeeSponsorshipTransaction =>
          AssetTransactionsDiff.cancelSponsorship(s, settings, currentBlockTimestamp, currentBlockHeight)(ctx)
