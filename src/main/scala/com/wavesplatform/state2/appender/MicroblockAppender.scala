...
object MicroblockAppender extends ScorexLogging with Instrumented {
  ...
  def apply(checkpoint: CheckpointService, history: History, blockchainUpdater: BlockchainUpdater, utxStorage: UtxPool, scheduler: Scheduler)(
    ...
    Task(
      measureSuccessful(
        ...
        for {
          _ <- Either.cond(
            checkpoint.isBlockValid(microBlock.totalResBlockSig, history.height + 1),
            (),
