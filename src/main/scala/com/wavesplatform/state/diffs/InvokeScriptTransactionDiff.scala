...
import com.wavesplatform.lang.v1.evaluator.{ContractEvaluator, Log, LogItem, ScriptResult}
...
import com.wavesplatform.transaction.smart.script.{Script, ScriptExecutionTrace, ScriptRunner}
...
object InvokeScriptTransactionDiff {
  def apply(blockchain: Blockchain, height: Int)(tx: InvokeScriptTransaction): TracedDiffResult = {
    ...
    def evalContract(contract: DApp): (Either[(ExecutionError, Log), ScriptResult], ScriptExecutionTrace) = {
      ...
      val result = for {
        directives <- DirectiveSet(V3, Account, DAppType).leftMap((_, List[LogItem]()))
    ...
    sc match {
      case Some(ContractScriptImpl(_, contract, _)) =>
        ...
        contract.cfs.find(_.u.name == functionName) match {
          ...
          case Some(_) =>
            val (result, trace) = evalContract(contract)
            val diff = result
                  .leftMap { case (error, log) => ScriptExecutionError(error, log, isTokenScript = false) }
                  .flatMap {
