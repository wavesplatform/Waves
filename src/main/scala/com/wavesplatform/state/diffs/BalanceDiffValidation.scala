...
object BalanceDiffValidation extends ScorexLogging with Instrumented {
  ...
  private val stats = Kamon.metrics.entity(TxMetrics, "balance-validation-stats")

  def apply(b: Blockchain, currentHeight: Int, fs: FunctionalitySettings)(d: Diff): Either[AccountBalanceError, Diff] = {
    val (_, (_, tx, _)) = d.transactions.head
    measureAndIncSuccessful(stats.processingTime(tx.builder.typeId), stats.processed(tx.builder.typeId)) {
