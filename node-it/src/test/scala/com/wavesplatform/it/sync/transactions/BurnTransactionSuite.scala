...
class BurnTransactionSuite extends BaseTransactionSuite {
  ...
  test("burning assets changes issuer's asset balance; issuer's waves balance is decreased by fee") {
    for (v <- burnTxSupportedVersions) {
      ...
      val issuedAssetId =
        miner.issue(firstKeyPair, s"name+$v", "description", issueAmount, decimals, reissuable = false, fee = issueFee, waitForTx = true).id
      ...
      val burnTx = miner.burn(firstKeyPair, issuedAssetId, issueAmount / 2, minFee, version = v, waitForTx = true)

      if (v > 2) {
        burnTx.chainId shouldBe Some(AddressScheme.current.chainId)
        miner.transactionInfo[BurnTransactionInfo](burnTx.id).chainId shouldBe Some(AddressScheme.current.chainId)
      }
      miner.transactionInfo[BurnTransactionInfo](burnTx.id).amount shouldBe issueAmount / 2
      ...
      miner.burn(firstKeyPair, issuedAssetId, issueAmount / 2, minFee, version = v, waitForTx = true).id
  ...
  test("can burn non-owned asset; issuer asset balance decreased by transfer amount; burner balance decreased by burned amount") {
    for (v <- burnTxSupportedVersions) {
      ...
      val issuedAssetId =
        miner.issue(firstKeyPair, s"name+$v", "description", issuedQuantity, decimals, reissuable = false, issueFee, waitForTx = true).id

      miner.assertAssetBalance(firstAddress, issuedAssetId, issuedQuantity)
      miner.transfer(firstKeyPair, secondAddress, transferredQuantity, minFee, issuedAssetId.some, waitForTx = true).id

      miner.assertAssetBalance(firstAddress, issuedAssetId, issuedQuantity - transferredQuantity)
      miner.assertAssetBalance(secondAddress, issuedAssetId, transferredQuantity)

      miner.burn(secondKeyPair, issuedAssetId, transferredQuantity, minFee, v, waitForTx = true).id
      miner.assertAssetBalance(secondAddress, issuedAssetId, 0)
      ...
      assertBadRequestAndMessage(
        miner.transfer(secondKeyPair, firstAddress, transferredQuantity / 2, minFee, issuedAssetId.some).id,
  ...
  test("issuer can't burn more tokens than he own") {
    for (v <- burnTxSupportedVersions) {
      ...
      val issuedAssetId =
        miner.issue(firstKeyPair, s"name+$v", "description", issuedQuantity, decimals, reissuable = false, issueFee).id

      nodes.waitForHeightAriseAndTxPresent(issuedAssetId)

      miner.assertAssetBalance(firstAddress, issuedAssetId, issuedQuantity)
      assertBadRequestAndMessage(miner.burn(secondKeyPair, issuedAssetId, burnedQuantity, minFee, v).id, "Accounts balance errors")
    }
  }
  ...
  test("user can't burn more tokens than he own") {
    for (v <- burnTxSupportedVersions) {
      ...
      val issuedAssetId = miner.issue(firstKeyPair, s"name+$v", "description", issuedQuantity, decimals, reissuable = false, issueFee).id

      miner.waitForTransaction(issuedAssetId)
      miner.assertAssetBalance(firstAddress, issuedAssetId, issuedQuantity)

      val transferId = miner.transfer(firstKeyPair, secondAddress, transferredQuantity, minFee, issuedAssetId.some).id
  ...
  test("non-owner can burn asset after reissue") {
    for (v <- burnTxSupportedVersions) {
      ...
      val issuedAssetId = miner.issue(firstKeyPair, s"name+$v", "description", issuedQuantity, decimals, reissuable = true, issueFee).id
      ...
      val burnOwnerTxTd = miner.burn(firstKeyPair, issuedAssetId, transferredQuantity, minFee, v).id
      ...
      val reissueId = miner.reissue(firstKeyPair, issuedAssetId, issuedQuantity, false, issueFee).id
      ...
      val burn1 = miner.burn(firstKeyPair, issuedAssetId, issuedQuantity, minFee, v).id
      miner.waitForTransaction(burn1)

      val burn2 = miner.burn(secondKeyPair, issuedAssetId, transferredQuantity, minFee, v).id
      ...
      assertBadRequestAndMessage(miner.reissue(firstKeyPair, issuedAssetId, issuedQuantity, true, issueFee).id, "Asset is not reissuable")
      assertBadRequestAndMessage(
        miner.transfer(secondKeyPair, thirdAddress, transferredQuantity / 2, minFee, issuedAssetId.some).id,
        "Attempt to transfer unavailable funds"
      )
      assertBadRequestAndMessage(
        miner.transfer(firstKeyPair, thirdAddress, transferredQuantity / 2, minFee, issuedAssetId.some).id,
        "Attempt to transfer unavailable funds"
      )
  ...
  test("send burn with quantity field") {
    val issuedAssetId =
      miner.issue(firstKeyPair, "name", "description", issueAmount, decimals, reissuable = false, fee = issueFee, waitForTx = true).id
    ...
    miner.signedBroadcast(json, waitForTx = true).id
  }
}
