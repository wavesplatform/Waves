...
class TransferNFTSuite extends BaseTransactionSuite with NTPTime {
  ...
  test("NFT should correctly be transferred via exchange transaction") {
    ...
    val buy  = Order.buy(sender = buyer, matcher = matcher, pair = pair.get, amount = 1, price = 1.waves, timestamp = ts, expiration = ts + Order.MaxLiveTime, matcherFee = matcherFee)
    val sell = Order.sell(sender = seller, matcher = matcher, pair = pair.get, amount = 1, price = 1.waves, timestamp = ts, expiration = ts + Order.MaxLiveTime, matcherFee = matcherFee)
