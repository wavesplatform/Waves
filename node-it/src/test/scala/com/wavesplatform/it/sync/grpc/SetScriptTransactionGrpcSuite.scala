  ...
  test("able to set script to account (multisig)") {
    for (v <- setScrTxSupportedVersions) {
      ...
      val scriptComplexity = Script
        .estimate(Script.fromBase64String(script.bytes().base64).explicitGet(), ScriptEstimatorV3(overhead = false), fixEstimateOfVerifier = true, useContractVerifierLimit = true)
        .explicitGet()
