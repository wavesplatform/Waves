...
class DataTransactionSuite extends BaseTransactionSuite with EitherValues {
  ...
  test("put and remove keys") {
    ...
    assertApiError(
      sender
        .broadcastData(sender.privateKey, updateAndRemoveDataEntries, calcDataFee(updateAndRemoveDataEntries, TxVersion.V1), version = TxVersion.V1),
    ...
    val removeDataTxId =
      sender.broadcastData(sender.privateKey, List(EmptyDataEntry("del")), calcDataFee(List(EmptyDataEntry("del")), TxVersion.V2)).id
  ...
  test("sender's waves balance is decreased by fee") {
    for (v <- dataTxSupportedVersions) {
      ...
      if (v > 2) {
        dataTx.chainId shouldBe Some(ChainId.global)
        sender.transactionInfo[TransactionInfo](dataTx.id).chainId shouldBe Some(ChainId.global)
      }
