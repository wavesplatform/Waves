...
class HodlContractTransactionSuite extends BaseTransactionSuite with CancelAfterFailure {
  ...
  test("setup contract account with waves") {
    sender
      .transfer(
        sender.address,
        recipient = contract.addressString,
        assetId = None,
        amount = 5.waves,
        fee = minFee,
        waitForTx = true
  ...
  test("setup caller account with waves") {
    sender
      .transfer(
        sender.address,
        recipient = caller.addressString,
        assetId = None,
        amount = 10.waves,
        fee = minFee,
        waitForTx = true
  ...
  test("set contract to contract account") {
    ...
    val setScriptId = sender.setScript(contract.addressString, Some(script), setScriptFee, waitForTx = true).id

    val acc0ScriptInfo = sender.addressScriptInfo(contract.addressString)
  ...
  test("caller deposits waves") {
    val balanceBefore = sender.accountBalances(contract.addressString)._1
    val invokeScriptId = sender
      .invokeScript(
        caller.addressString,
        dappAddress = contract.addressString,
        func = Some("deposit"),
        args = List.empty,
    ...
    sender.getDataByKey(contract.addressString, caller.addressString) shouldBe IntegerDataEntry(caller.addressString, 1.5.waves)
    val balanceAfter = sender.accountBalances(contract.addressString)._1
  ...
  test("caller can withdraw less than he owns") {
    ...
    sender.getDataByKey(contract.addressString, caller.addressString) shouldBe IntegerDataEntry(caller.addressString, 0.01.waves)
    ...
    stateChangesTransfers.address shouldBe caller.addressString
