...
class GeneratingBalanceSuite extends GrpcBaseTransactionSuite {
  ...
  test("Generating balance should be correct") {
    ...
    sender.grpc.broadcastTransfer(sender.privateKey, Recipient().withPublicKeyHash(recipientAddress), amount, minFee, 2, waitForTx = true)

    val afterTransferBalance = sender.grpc.wavesBalance(senderAddress)

    sender.grpc.broadcastTransfer(recipient, Recipient().withPublicKeyHash(senderAddress), amount - minFee, minFee, 2, waitForTx = true)
