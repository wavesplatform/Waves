...
class LeasingTransactionsSuite extends BaseTransactionSuite with CancelAfterFailure {
  ...
  test("leasing waves decreases lessor's eff.b. and increases lessee's eff.b.; lessor pays fee") {
    for (v <- leaseTxSupportedVersions) {
      ...
      if (v > 2) {
        createdLeaseTx.chainId shouldBe Some(ChainId.global)
        sender.transactionInfo[TransactionInfo](createdLeaseTx.id).chainId shouldBe Some(ChainId.global)
      }
  ...
  test("lease cancellation reverts eff.b. changes; lessor pays fee for both lease and cancellation") {
    ...
    for (v <- leaseTxSupportedVersions) {
      ...
      if (v > 2) {
        createdCancelLeaseTx.chainId shouldBe Some(ChainId.global)
        sender.transactionInfo[TransactionInfo](createdCancelLeaseTx.id).chainId shouldBe Some(ChainId.global)
      }
