...
class InvokeScriptWithSponsorshipSuite extends BaseTransactionSuite with CancelAfterFailure {
  ...
  test("_send waves to dApp and caller accounts") {
    val dAppTransferId   = sender.transfer(sender.address, dApp.addressString, 5.waves, minFee).id
    val callerTransferId = sender.transfer(sender.address, caller.addressString, 5.waves, minFee).id
  ...
  test("_issue and transfer assets") {
    dAppAsset = sender.issue(dApp.addressString, "dApp", "d", quantity, 0).id
    callerAsset = sender.issue(caller.addressString, "caller", "c", quantity, 0).id
    val script = Some(ScriptCompiler.compile("true").explicitGet()._1.bytes.value.base64)
    smartAsset = sender.issue(dApp.addressString, "Smart", "s", quantity, 0, script = script).id

    nodes.waitForHeightAriseAndTxPresent(callerAsset)
    sender.waitForTransaction(dAppAsset)

    val dAppToCallerId  = sender.transfer(dApp.addressString, caller.addressString, halfQuantity, minFee, Some(dAppAsset)).id
    val callerToDAppId  = sender.transfer(caller.addressString, dApp.addressString, halfQuantity, minFee, Some(callerAsset)).id
    val smartToCallerId = sender.transfer(dApp.addressString, caller.addressString, halfQuantity, smartMinFee, Some(smartAsset)).id
  ...
  test("_enable sponsorship") {
    val dAppSponsor   = sender.sponsorAsset(dApp.addressString, dAppAsset, 1).id
    val callerSponsor = sender.sponsorAsset(caller.addressString, callerAsset, 1).id
  ...
  test("_set scripts to dApp and caller account") {
    ...
    val dAppSetScriptTxId = sender.setScript(dApp.addressString, Some(dAppScript.bytes().base64)).id
    ...
    val callerScript        = ScriptCompiler.compile(s"""
          ...
          |}
        """.stripMargin).explicitGet()._1
    val callerSetScriptTxId = sender.setScript(caller.addressString, Some(callerScript.bytes().base64)).id

    nodes.waitForHeightAriseAndTxPresent(callerSetScriptTxId)
    sender.waitForTransaction(dAppSetScriptTxId)

    val dAppScriptInfo = sender.addressScriptInfo(dApp.addressString)
    ...
    val smartCallerScriptInfo = sender.addressScriptInfo(caller.addressString)
  ...
  test("required fee in sponsored assets considers scripts count") {
    dAppInitBalance = sender.accountBalances(dApp.addressString)._1
    callerInitBalance = sender.accountBalances(caller.addressString)._1
    ...
    sender.assetBalance(dApp.addressString, dAppAsset).balance shouldBe halfQuantity + (feeAmount - 10) + smartFeeAmount
    sender.assetBalance(dApp.addressString, callerAsset).balance shouldBe halfQuantity + paymentAmount
    sender.accountBalances(dApp.addressString)._1 shouldBe dAppInitBalance - 0.009.waves - 0.053.waves

    sender.assetBalance(caller.addressString, dAppAsset).balance shouldBe halfQuantity + (-feeAmount + 10) - smartFeeAmount
    sender.assetBalance(caller.addressString, callerAsset).balance shouldBe halfQuantity - paymentAmount
    sender.accountBalances(caller.addressString)._1 shouldBe callerInitBalance
  }
