...
class MassTransferSmartContractSuite extends BaseTransactionSuite with CancelAfterFailure {
  ...
  test("airdrop emulation via MassTransfer") {
    ...
    val unsigned =
      MassTransferTransaction
        .create(1.toByte, Waves, notMiner.publicKey, transfers, currTime, calcMassTransferFee(2) + smartFee, Array.emptyByteArray, Proofs.empty)
        .explicitGet()
    ...
    val unsignedToGov =
      MassTransferTransaction
        .create(1.toByte, Waves, notMiner.publicKey, transfersToGov, currTime, calcMassTransferFee(2) + smartFee, Array.emptyByteArray, Proofs.empty)
        .explicitGet()
    ...
    val unsignedToGovSecond =
      MassTransferTransaction
        .create(1.toByte, Waves, notMiner.publicKey, transfersToGov, System.currentTimeMillis(), calcMassTransferFee(2) + smartFee, Array.emptyByteArray, Proofs.empty)
        .explicitGet()
