...
class DataTransactionGrpcSuite extends GrpcBaseTransactionSuite {
  ...
  test("try to send tx above limits of key, value and size") {
    ...
    assertGrpcError(sender.putData(firstAcc, tooBigKeyDataEntry, calcDataFee(tooBigKeyDataEntry)), s"Too big sequences requested", Code.INVALID_ARGUMENT)
    assertGrpcError(sender.putData(firstAcc, List(DataEntry("", DataEntry.Value.BoolValue(false))), 1.waves), "Empty key found", Code.INVALID_ARGUMENT)
    assertGrpcError(
    ...
    assertGrpcError(sender.putData(firstAcc, extraValueData, calcDataFee(extraValueData)), "Too big sequences requested", Code.INVALID_ARGUMENT)
    ...
    assertGrpcError(sender.putData(firstAcc, largeBinData, calcDataFee(largeBinData)), "Too big sequences requested", Code.INVALID_ARGUMENT)

    val largeStrData = List.tabulate(5)(n => DataEntry(s"key$n", DataEntry.Value.StringValue("A" * MaxValueSize)))
    assertGrpcError(sender.putData(firstAcc, largeStrData, calcDataFee(largeStrData)), "Too big sequences requested", Code.INVALID_ARGUMENT)

    val tooManyEntriesData = List.tabulate(MaxEntryCount + 1)(n => DataEntry(s"key$n", DataEntry.Value.IntValue(10)))
    assertGrpcError(sender.putData(firstAcc, tooManyEntriesData, calcDataFee(tooManyEntriesData)), "Too big sequences requested", Code.INVALID_ARGUMENT)
  }
