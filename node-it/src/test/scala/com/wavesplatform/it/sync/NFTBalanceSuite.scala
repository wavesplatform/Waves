...
class NFTBalanceSuite
  ...
  protected override def beforeAll(): Unit = {
    ...
    val fundAndIssue =
      for {
        ...
        fundTx <- node.transfer(node.address, issuer.addressString, 1000.waves, 0.001.waves)
  ...
  "after activation" - {
    "returns only non-nft portfolio on /balance/{address}" in {
      ...
      val assertion =
        getPortfolio(node, issuer.addressString) map { ids =>
    ...
    "returns issue transactions on /nft/{address}/limit/{limit}" in {
      ...
      val assertion =
        getNFTPage(node, issuer.addressString, 1000, None) map { ids =>
          ids.toSet shouldBe expectedIds.toSet
        }
    ...
    "returns only nft with balance > 0 on /nft/{address}/limit/{limit}" in {
      ...
      val assertion = for {
        ...
        issuerNFTs <- getNFTPage(node, issuer.addressString, 1000, None)
        otherNFTs  <- getNFTPage(node, other.addressString, 1000, None)
      } yield {
  ...
  "pagination" - {
    "works" in {
      ...
      val assertion = for {
        pagedIds    <- getNFTPaged(node, issuer.addressString, 10).map(_.toSet)
        nonPagedIds <- getNFTPage(node, issuer.addressString, 1000, None).map(_.toSet)
      } yield {
    ...
    "returns error on wrong limit" in {
      val assertion = getNFTPage(node, issuer.addressString, 10000000, None)
    ...
    "returns error on wrong base58 in after" in {
      val assertion = getNFTPage(node, issuer.addressString, 100, Some("wr0ngbase58str1ng"))
        .map(_ => org.scalatest.Assertions.fail("BadRequest expected"))
        .recoverWith {
          case ex: Throwable =>
            Future.successful {
