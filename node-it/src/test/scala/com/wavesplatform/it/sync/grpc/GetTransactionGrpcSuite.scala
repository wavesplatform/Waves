...
class GetTransactionGrpcSuite extends GrpcBaseTransactionSuite {
  ...
  test("get transaction by sender, by recipient, by sender&recipient and id") {
    val txId = PBTransactions.vanilla(
      miner.broadcastTransfer(firstAcc, Recipient().withPublicKeyHash(secondAddress), transferAmount, minFee, waitForTx = true)
    ).explicitGet().id().toString
    val transactionBySenderAndId = miner.getTransaction(sender = firstAddress, id = txId).getTransaction
    val transactionByRecipientAndId = miner.getTransaction(recipient = Some(Recipient().withPublicKeyHash(secondAddress)), id = txId).getTransaction
    val transactionBySenderRecipientAndId = miner.getTransaction(sender = firstAddress, recipient = Some(Recipient().withPublicKeyHash(secondAddress)), id = txId).getTransaction
  ...
  test("get multiple transactions") {
    val txs = List.fill(10)(miner.broadcastTransfer(thirdAcc, Recipient().withPublicKeyHash(secondAddress), transferAmount / 10, minFee, waitForTx = true))
    val txsIds = txs.map(tx => PBTransactions.vanilla(tx).explicitGet().id().toString)

    val transactionsByIds = miner.getTransactionSeq(txsIds, sender = thirdAddress, recipient = Some(Recipient().withPublicKeyHash(secondAddress)))
