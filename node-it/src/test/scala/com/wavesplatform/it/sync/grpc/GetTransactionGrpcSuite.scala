...
class GetTransactionGrpcSuite extends GrpcBaseTransactionSuite {
  ...
  test("get transaction by sender, by recipient, by sender&recipient and id") {
    ...
    val transactionByRecipientAndId = sender.grpc.getTransaction(recipient = Some(Recipient().withAddress(secondAddress)), id = txId).getTransaction
    val transactionBySenderRecipientAndId = sender.grpc.getTransaction(sender = firstAddress, recipient = Some(Recipient().withAddress(secondAddress)), id = txId).getTransaction
  ...
  test("get multiple transactions") {
    val txs = List.fill(10)(sender.grpc.broadcastTransfer(firstAcc, Recipient().withAddress(secondAddress), transferAmount / 10, minFee, waitForTx = true))
    val txsIds = txs.map(tx => PBTransactions.vanilla(tx).explicitGet().id().base58)

    val transactionsByIds = sender.grpc.getTransactionSeq(txsIds, sender = firstAddress, recipient = Some(Recipient().withAddress(secondAddress)))
