...
class ExchangeTransactionSuite extends BaseTransactionSuite with NTPTime {
  ...
  test("exchange tx with orders v3") {
    ...
    nodes.waitForHeightAriseAndTxPresent(smartAssetId.toString)
    ...
    for ((o1ver, o2ver, matcherFeeOrder1, matcherFeeOrder2) <- Seq(
      ...
      if (matcherFeeOrder1 == Waves && matcherFeeOrder2 == IssuedAsset(assetId)) {
        ...
        assetBalanceBefore = sender.assetBalance(secondAddress, smartAssetId.toString).balance
        sender.transfer(buyer.address, seller.address, 300000, minFee + smartFee, Some(smartAssetId.toString), waitForTx = true)
      }
      ...
      if (matcherFeeOrder1 == Waves && matcherFeeOrder2 == IssuedAsset(assetId)) {
        ...
        sender.assetBalance(secondAddress, smartAssetId.toString).balance shouldBe assetBalanceBefore
      }
    }
  }
}
