...
class ScriptExecutionErrorSuite extends BaseTransactionSuite with CancelAfterFailure {
  ...
  test("custom throw message") {
    ...
    assertBadRequestAndResponse(
      sender.signedBroadcast(CreateAliasTransaction.selfSigned(Transaction.V2, ts, acc2, alias, minFee).explicitGet().json()),
  ...
  test("wrong type of script return value") {
    ...
    assertBadRequestAndResponse(
      sender.signedBroadcast(
        TransferTransaction
          .selfSigned(2.toByte, ts, acc0, acc1.toAddress, Waves, 1000, Waves, minFee, Array())
          .explicitGet()
          .json()
      ),
      "not a boolean"
