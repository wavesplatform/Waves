...
class FailedTransactionGrpcSuite extends GrpcBaseTransactionSuite with FailedTransactionSuiteLike[PBSignedTransaction] {
  ...
  protected override def beforeAll(): Unit = {
      ...
      .vanillaUnsafe(
        sender
          .broadcastIssue(
            ...
            script = Right(ScriptCompiler.compile("true", ScriptEstimatorV3.instance).toOption.map(_._1)),
            waitForTx = true
          )
      )
      .id()
    ...
    val scriptTextV4 =
      s"""
         ...
         |
        """.stripMargin
    val script = ScriptCompiler.compile(scriptTextV4, ScriptEstimatorV3.instance).explicitGet()._1
  ...
  test("ExchangeTransaction: failed exchange tx when asset script fails") {
    ...
    val initScript: Either[Array[Byte], Option[Script]] = Right(ScriptCompiler.compile("true", ScriptEstimatorV3.instance).toOption.map(_._1))
