...
class SponsorFeeTransactionGrpcSuite extends GrpcBaseTransactionSuite {
  ...
  test("able to make transfer with sponsored fee") {
    for (v <- sponsorFeeTxSupportedVersions) {
      ...
      sender.grpc.broadcastSponsorFee(sponsor, sponsoredAssetMinFee, fee = sponsorFee, version = v, waitForTx = true)
  ...
  test("only issuer is able to sponsor asset") {
    for (v <- sponsorFeeTxSupportedVersions) {
      ...
      val sponsoredAssetMinFee = Some(Amount.of(ByteString.copyFrom(Base58.decode(sponsoredAssetId)), token))
      assertGrpcError(
        sender.grpc.broadcastSponsorFee(alice, sponsoredAssetMinFee, fee = sponsorFee, version = v),
  ...
  test("sponsor is able to cancel sponsorship") {
    for (v <- sponsorFeeTxSupportedVersions) {
      ...
      val sponsoredAssetMinFee = Some(Amount.of(ByteString.copyFrom(Base58.decode(sponsoredAssetId)), token))
      sender.grpc.broadcastSponsorFee(alice, sponsoredAssetMinFee, fee = sponsorFee, version = v, waitForTx = true)
      ...
      sender.grpc.broadcastSponsorFee(alice, sponsoredAssetNullMinFee, fee = sponsorFee, version = v, waitForTx = true)
  ...
  test("sponsor is able to update amount of sponsored fee") {
    for (v <- sponsorFeeTxSupportedVersions) {
      ...
      sender.grpc.broadcastSponsorFee(sponsor, sponsoredAssetUpdatedMinFee, fee = sponsorFee, version = v, waitForTx = true)
