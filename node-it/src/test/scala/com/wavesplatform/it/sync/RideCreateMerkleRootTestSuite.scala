...
class RideCreateMerkleRootTestSuite
  ...
  test("Ride createMerkleRoot") {
    ...
    val tx1 = node.broadcastTransfer(node.privateKey, sender.address, setScriptFee, minFee, None, None, version = TxVersion.V3, waitForTx = false)
    val txId1 = tx1.id
    val tx2 = node.broadcastTransfer(node.privateKey, node.address, 1, minFee, None, None, version = TxVersion.V3, waitForTx = false)
    val txId2 = tx2.id
    val tx3 = node.broadcastTransfer(node.privateKey, node.address, 1, minFee, None, None, version = TxVersion.V3, waitForTx = false)
    val txId3 = tx3.id
    val tx4 = node.broadcastTransfer(node.privateKey, node.address, 1, minFee, None, None, version = TxVersion.V3, waitForTx = false)
    val txId4 = tx4.id
    val tx5 = node.broadcastTransfer(node.privateKey, node.address, 1, minFee, None, None, version = TxVersion.V3, waitForTx = false)
    val txId5 = tx5.id
    ...
    val natives = Seq(tx1, tx2, tx3, tx4, tx5).map(tt).map(t => Base58.encode(t.id()) -> t).toMap
    ...
    for(p <- proofs) {
      node.invokeScript(
        ...
        sender.address,
        func = Some("foo"),
        args = List(ARR(p.merkleProof.map(v => CONST_BYTESTR(Base58.decode(v)).explicitGet()).toIndexedSeq, false).explicitGet(),
                    CONST_BYTESTR(Merkle.hash(natives(p.id).bytes())).explicitGet(),
