...
class AccountDataDiskCacheTestSuite extends DiskCacheWithHistoryTestSuite[CacheKey.AccountData, DataEntry[?]] {
  ...
  protected override def test(f: (RideDbAccess, DiskCache[CacheKey.AccountData, DataEntry[?]]) => Unit): Unit = withDb { db =>
