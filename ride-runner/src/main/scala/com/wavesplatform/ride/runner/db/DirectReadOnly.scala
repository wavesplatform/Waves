...
class DirectReadOnly(db: RocksDB) extends ReadOnly {
  override def get[V](key: Key[V]): V = {
    val bytes = db.get(key.columnFamilyHandle.getOrElse(db.getDefaultColumnFamily), key.keyBytes)
  ...
  override def iterateOverPrefixContinue(seekKeyBytes: Array[Byte], cfh: Option[ColumnFamilyHandle] = None)(f: DBEntry => Boolean): Unit = {
    ...
    Using.resource(db.newIterator(cfh.getOrElse(db.getDefaultColumnFamily))) { iter =>
      iter.seek(seekKeyBytes)
      loop(iter)
    }
  }

  override def prefixExists(prefix: Array[Byte]): Boolean = Using.resource(db.newIterator()) { iter =>
    iter.seek(prefix)
