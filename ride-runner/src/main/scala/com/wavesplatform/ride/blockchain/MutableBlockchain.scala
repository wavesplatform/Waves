...
import com.wavesplatform.ride.blockchain.MutableBlockchain.CachedData
import com.wavesplatform.settings.BlockchainSettings
import com.wavesplatform.state.reader.LeaseDetails
import com.wavesplatform.state.{AccountScriptInfo, AssetDescription, AssetScriptInfo, BalanceSnapshot, Blockchain, DataEntry, Height, LeaseBalance, Portfolio, TxMeta, VolumeAndFee}
...
class MutableBlockchain(override val settings: BlockchainSettings, blockchainApi: BlockchainGrpcApi) extends Blockchain with ScorexLogging {
  private type TagT = Blockchain // TODO here should be passed the tagged BlockchainStorage, so we can get rid of it
...
//  private val data = mutable.AnyRefMap.empty[Address, mutable.AnyRefMap[String, CachedData[DataEntry[_]]]]
  val data = RideData[(Address, String), DataEntry[?], TagT](Function.tupled(blockchainApi.getAccountDataEntry))
