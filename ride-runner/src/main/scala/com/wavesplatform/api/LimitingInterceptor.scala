...
class LimitingInterceptor(maxConcurrentCalls: Int) extends ClientInterceptor {
  ...
  override def interceptCall[ReqT, RespT](
    ...
    if (semaphore.availablePermits() == 0) GrpcStats.blockedCall(GrpcMethod.of(method)).increment()
    semaphore.acquire()
    new ForwardingClientCall.SimpleForwardingClientCall[ReqT, RespT](channel.newCall(method, callOptions)) {
