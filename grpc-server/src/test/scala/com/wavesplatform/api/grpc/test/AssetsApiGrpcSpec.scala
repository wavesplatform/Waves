...
import com.wavesplatform.api.grpc.{AssetInfoResponse, AssetRequest, AssetsApiGrpcImpl, NFTRequest, NFTResponse, ScriptData}
...
class AssetsApiGrpcSpec extends FreeSpec with BeforeAndAfterAll with DiffMatchers with WithDomain with GrpcApiHelpers {
  ...
  "GetInfo" - {
    "NODE-992. GetInfo for existing asset should return right result" in withDomain(
      ...
      d.liquidAndSolidAssert { () =>
        val result = Await.result(grpcApi.getInfo(AssetRequest(ByteString.copyFrom(assetIssue.id.value().arr))), timeout)
    ...
    "NODE-993. GetInfo for existing smart asset should return right result" in withDomain(
      RideV6.addFeatures(BlockchainFeatures.ReduceNFTFee),
      AddrWithBalance.enoughBalances(sender)
    ) { d =>
      val grpcApi    = getGrpcApi(d)
      val script = Some(TestCompiler(V6).compileAsset(script = "true"))
      val assetIssue = TxHelpers.issue(sender, 1, name = "asset", description = "some asset",
       script = script)
  ...
  "GetNFTList" - {
    "NODE-940. GetNFTList should work" in withDomain(RideV6.addFeatures(BlockchainFeatures.ReduceNFTFee), AddrWithBalance.enoughBalances(sender)) {
      d =>
        val grpcApi = getGrpcApi(d)
...
//todo after
    "NODE-999. GetNftList param after should work" in withDomain(
      RideV6.addFeatures(BlockchainFeatures.ReduceNFTFee),
      AddrWithBalance.enoughBalances(sender)
    ) { d =>
      val grpcApi = getGrpcApi(d)
      ...
      d.liquidAndSolidAssert { () =>
        val (observer, result) = createObserver[NFTResponse]
        grpcApi.getNFTList(
          NFTRequest.of(ByteString.copyFrom(sender.toAddress.bytes), 5, ByteString.copyFrom(nftIssues(dropIdx - 1).asset.id.arr)),
          observer
        )
        result.runSyncUnsafe() shouldBe nftIssues.drop(dropIdx).zipWithIndex.map { case (nftTx, i) =>
          NFTResponse.of(
            ByteString.copyFrom(nftTx.asset.id.arr),
            Some(
              AssetInfoResponse.of(
    ...
    "NODE-1000. GetNftList should return empty response on address without NFT" in withDomain(
      RideV6.addFeatures(BlockchainFeatures.ReduceNFTFee),
      AddrWithBalance.enoughBalances(sender)
    ) { d =>
      val grpcApi = getGrpcApi(d)
    ...
    "NODE-1004. GetNftList limit 0 should return empty list" in withDomain(
      RideV6.addFeatures(BlockchainFeatures.ReduceNFTFee),
      AddrWithBalance.enoughBalances(sender)
    ) { d =>
      val grpcApi = getGrpcApi(d)
    ...
    // todo
    "NODE-1005. GetNftList max limit should return equal number of assets" in withDomain(
      RideV6.addFeatures(BlockchainFeatures.ReduceNFTFee),
      AddrWithBalance.enoughBalances(sender)
    ) { d =>
      val grpcApi = getGrpcApi(d)

      val nftIssues: Seq[IssueTransaction] = (1 to 2).flatMap { bId =>
      ...
      val (observer, result) = createObserver[NFTResponse]
      grpcApi.getNFTList(
        NFTRequest.of(ByteString.copyFrom(sender.toAddress.bytes), 1001, ByteString.EMPTY),
        observer
      )
      result.runSyncUnsafe() shouldBe nftIssues.take(1001).zipWithIndex.map { case (nftTx, i) =>
        NFTResponse.of(
          ByteString.copyFrom(nftTx.asset.id.arr),
          Some(
            AssetInfoResponse.of(
              ...
              None,
              0,
              sequenceInBlock = 1 + (i % 1000),
              issueHeight = 2 + i / 1000
            )
          )
        )
      // }
