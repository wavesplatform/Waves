...
class BlockchainUpdaterBurnTest extends PropSpec with DomainScenarioDrivenPropertyCheck {
  ...
  val preconditions: Gen[Setup] = for {
    ...
    burn: BurnTransaction       = TxHelpers.burn(issue.asset, quantity / 2, alice, version = TxVersion.V1, timestamp = ts)
    reissue: ReissueTransaction = TxHelpers.reissue(issue.asset, alice, burn.quantity.value, version = TxVersion.V1,  timestamp = ts)
