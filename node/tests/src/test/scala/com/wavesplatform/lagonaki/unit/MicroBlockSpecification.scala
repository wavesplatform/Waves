...
class MicroBlockSpecification extends FunSuite with Matchers with MockFactory with ShouldVerb {
  ...
  test("MicroBlock with txs bytes/parse roundtrip") {
    ...
    val tr: TransferTransaction  = SignedTx.transfer(1.toByte, sender, gen.toAddress, Waves, 5, Waves, 2, ByteStr.empty,  ts + 1)
    val assetId                  = IssuedAsset(ByteStr(Array.fill(AssetIdLength)(Random.nextInt(100).toByte)))
    val tr2: TransferTransaction = SignedTx.transfer(1.toByte, sender, gen.toAddress, assetId, 5, Waves, 2, ByteStr.empty,  ts + 2)
  ...
  test("MicroBlock cannot contain more than Miner.MaxTransactionsPerMicroblock") {
    ...
    val transaction  = SignedTx.transfer(1.toByte, sender, gen.toAddress, Waves, 5, Waves, 1000, ByteStr.empty,  System.currentTimeMillis())
