  ...
  property("Contract payment should be positive") {
    forAll(for {
      ...
      case (acc, _, genesis, setScript, ci, asset, master, ts) =>
        val t = SignedTx.transfer(2.toByte, master, acc.toAddress, IssuedAsset(asset.id()), asset.quantity / 10, Waves, enoughFee, ByteStr.empty, ts)
    ...
    val failedTxScenario =
      for {
        ...
        i2Tx = IssueTransaction
          ...
          .explicitGet()
        tTx = SignedTx.transfer(TxVersion.V3, other, invoker.toAddress, sTx.asset, i1Tx.quantity, Waves, enoughFee, ByteStr.empty, ts)
        funcBinding                     <- funcNameGen
  ...
  property(
    ...
    val scenario =
      for {
        ...
        tTx = SignedTx.transfer(TxVersion.V3, other, master.toAddress, sponsoredAsset, iTx.quantity / 2, Waves, enoughFee, ByteStr.empty,  ts)
        wavesFee <- ciFee(1)
