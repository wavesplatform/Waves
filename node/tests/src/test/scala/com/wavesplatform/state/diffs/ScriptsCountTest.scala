...
class ScriptsCountTest extends PropSpec with PropertyChecks with WithState with TransactionGen with NoShrink with Inside {
  ...
  property("check pre-Ride4DApps scripts run count") {
    forAll(for {
      ...
      tr1  = SignedTx.transfer(2.toByte, master, acc.toAddress, Waves, 10000000000L, Waves, fee, ByteStr.empty, timestamp)
      tr2  = SignedTx.transfer(2.toByte, master, acc.toAddress, IssuedAsset(issueScr.id()), 1000000000L, Waves, fee, ByteStr.empty, timestamp)
      mt1  = MassTransferTransaction.selfSigned(1.toByte, master, Waves, List(ParsedTransfer(acc.toAddress, 1)), fee, timestamp, ByteStr.empty)
        .explicitGet()
