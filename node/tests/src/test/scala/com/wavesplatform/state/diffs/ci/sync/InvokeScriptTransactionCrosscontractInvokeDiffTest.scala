  ...
  property("Crosscontract call - internal invoke state update") {
    ...
    val scenario =
      for {
        ...
        ssTxMain     = TxHelpers.setScript(mainAcc, scriptMain.get, fee, 1.toByte)
        ssTxSecond   = TxHelpers.setScript(secondAcc, scriptSecond.get, fee, 1.toByte)

        dataEntry    = StringDataEntry(invokeEntry2Key, "strData")
        dataTxSecond = TxHelpers.data(secondAcc, Seq(dataEntry), fee)

        dataEntry2    = StringDataEntry(invokeEntry3Key, "deleted entry")
        dataTxSecond2 = TxHelpers.data(secondAcc, Seq(dataEntry2), fee)

        fc       = Terms.FUNCTION_CALL(FunctionHeader.User("foo"), List.empty)
        payments = List(Payment(10L, Waves))
        invokeTx = InvokeScriptTransaction
          .create(TxVersion.V3, invoker.publicKey, mainAcc.toAddress, Some(fc), payments, fee, Waves, ts + 10)
  ...
  property("Crosscontract call - same contract internal invoke - state update") {
    ...
    val scenario =
      for {
        ...
        ssTxMain = TxHelpers.setScript(mainAcc, contractMain, fee, 1.toByte)

        dataEntry  = StringDataEntry(invokeEntry2Key, "strData")
        dataTxMain = TxHelpers.data(mainAcc, Seq(dataEntry), fee)

        dataEntry2  = StringDataEntry(invokeEntry3Key, "deleted entry")
        dataTxMain2 = TxHelpers.data(mainAcc, Seq(dataEntry2), fee)
  ...
  property("Crosscontract call - multiple internal invokes - state update") {
    ...
    val scenario =
      for {
        ...
        ssTxMain     = TxHelpers.setScript(mainAcc, scriptMain.get, fee, 1.toByte)
        ssTxSecond   = TxHelpers.setScript(secondAcc, scriptSecond.get, fee, 1.toByte)
        ssTxThird    = TxHelpers.setScript(thirdAcc, scriptThird.get, fee, 1.toByte)

        fc       = Terms.FUNCTION_CALL(FunctionHeader.User("foo"), List.empty)
        payments = List(Payment(10L, Waves))
        invokeTx = InvokeScriptTransaction
          .create(TxVersion.V3, invoker.publicKey, mainAcc.toAddress, Some(fc), payments, fee * 100, Waves, ts + 10)
          .map(_.signWith(invoker.privateKey))
          .explicitGet()
      } yield (
