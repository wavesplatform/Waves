...
class RollbackSpec extends FreeSpec with Matchers with WithDomain with TransactionGen with PropertyChecks with NoShrink {
  ...
  private def transfer(sender: KeyPair, recipient: Address, amount: Long) =
    SignedTx.transfer(1.toByte, sender, recipient, Waves, amount, Waves, 1, ByteStr.empty, nextTs)
        ...
        withDomain() { d =>
          ...
          d.appendBlock(
            TestBlock.create(
              nextTs,
              d.lastBlockId,
              Seq(SignedTx.transfer(1.toByte, sender, recipient.toAddress, IssuedAsset(issueTransaction.id()), assetAmount, Waves, 1, ByteStr.empty,  nextTs))
            )
          )
