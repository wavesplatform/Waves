...
class NotaryControlledTransferScenarioTest extends PropSpec with PropertyChecks with WithState with TransactionGen with NoShrink {
  private val preconditions =
    for {
      ...
      transferFromCompanyToA = SignedTx.transfer(1.toByte, company, accountA.toAddress, assetId, 1, Waves, 1000, ByteStr.empty, ts + 20)

      transferFromAToB = SignedTx.transfer(1.toByte, accountA, accountB.toAddress, assetId, 1, Waves, 1000, ByteStr.empty, ts + 30)
