...
class AssetTransactionsDiffTest
  ...
  property("Can not total issue > long.max") {
    val setup = for {
      ...
      transfer =SignedTx.transfer(1.toByte, issuer, holder.toAddress, assetId, quantity - 1, Waves, fee, ByteStr(attachment), timestamp)
  ...
  def genesisIssueTransferReissue(
    ...
    for {
      ...
      transfer = SignedTx.transfer(TxVersion.V1, accountA, accountB.toAddress, assetId, issue.quantity, Waves, smallFee, ByteStr.empty,  timestamp + 2)
