...
trait BlocksTransactionsHelpers { self: TransactionGen =>
  object QuickTX {
    ...
    def transfer(
      ...
      for {
        timestamp <- timestamp
      } yield SignedTx.transfer(1.toByte, from, to, Waves, amount, Waves, FeeAmount, ByteStr.empty, timestamp)
    ...
    def transferV2(
      ...
      for {
        timestamp <- timestamp
      } yield SignedTx.transfer(2.toByte, from, to, Waves, amount, Waves, FeeAmount, ByteStr.empty, timestamp)
    ...
    def transferAsset(
      ...
      for {
        timestamp <- timestamp
      } yield SignedTx.transfer(1.toByte, from, to, asset, amount, Waves, FeeAmount, ByteStr.empty, timestamp)
