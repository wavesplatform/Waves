 ...
 "minSponsoredAssetFee": null
  ...
  property("miner receives one satoshi less than sponsor pays") {
    val setup = for {
      ...
      transfer = SignedTx.transfer(1.toByte, acc, acc.toAddress, Waves, 1, feeAsset = IssuedAsset(issue.id()), minFee, ByteStr.empty,  ts)
  ...
  property("miner receives one satoshi more than sponsor pays") {
    val setup = for {
      ...
      sponsor   = SponsorFeeTransaction.selfSigned(1.toByte, acc, IssuedAsset(issue.id()), Some(minFee), One, ts).explicitGet()
      transfer1 = SignedTx.transfer(1.toByte, acc, acc.toAddress, Waves, 1, feeAsset = IssuedAsset(issue.id()), minFee + 7, ByteStr.empty, ts)
      transfer2 = SignedTx.transfer(1.toByte, acc, acc.toAddress, Waves, 1, feeAsset = IssuedAsset(issue.id()), minFee + 9, ByteStr.empty, ts)
  ...
  property("sponsorship changes in the middle of a block") {
    val setup = for {
      ...
      transfer1 = SignedTx.transfer(1.toByte, acc, acc.toAddress, Waves, 1, IssuedAsset(issue.id()), fee = minFee, ByteStr.empty,  ts)
      sponsor2  = SponsorFeeTransaction.selfSigned(1.toByte, acc, IssuedAsset(issue.id()), Some(minFee * 10), One, ts).explicitGet()
      transfer2 = SignedTx.transfer(1.toByte, acc, acc.toAddress, Waves, 1, IssuedAsset(issue.id()), fee = minFee * 10, ByteStr.empty,  ts)
