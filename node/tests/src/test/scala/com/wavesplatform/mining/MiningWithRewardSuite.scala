  ...
  private def withEnv(bps: Seq[BlockProducer], txs: Seq[TransactionProducer] = Seq(), settings: WavesSettings = MiningWithRewardSuite.settings)(
    ...
    resources(settings).use { case (blockchainUpdater, _) =>
      for {
        ...
        _ <- Task {
          blockchainUpdater.processBlock(genesisBlock, genesisBlock.header.generationSignature, snapshot = None, generatorBalances = Map.empty)
        }
        blocks = bps.foldLeft {
        ...
        added <- Task.traverse(blocks.reverse) { b =>
          Task(blockchainUpdater.processBlock(b, b.header.generationSignature, snapshot = None, generatorBalances = Map.empty))
        }
