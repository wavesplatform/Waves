  ...
  it should "generate valid blocks with transactions of version 4" in {
    val bps: Seq[BlockProducer] = Seq(
      (ts, reference, account) => {
        ...
        val tx1 = SignedTx.transfer(2.toByte, account, recipient1, Waves, 10 * Constants.UnitsInWave, Waves, 400000, ByteStr.empty, ts)
        val tx2 = SignedTx.transfer(2.toByte, account, recipient2, Waves, 5 * Constants.UnitsInWave, Waves, 400000, ByteStr.empty, ts)
    ...
    val txs: Seq[TransactionProducer] = Seq(
      (ts, account) => {
        val recipient1 = createAccount.toAddress
        SignedTx.transfer(2.toByte, account, recipient1, Waves, 10 * Constants.UnitsInWave, Waves, 400000, ByteStr.empty, ts)
      }
    )
