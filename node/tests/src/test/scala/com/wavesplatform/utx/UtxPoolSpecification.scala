...
class UtxPoolSpecification
  ...
  private def transfer(sender: KeyPair, maxAmount: Long, time: Time) =
    ...
    } yield SignedTx.transfer(1.toByte, sender, recipient.toAddress, Waves, amount, Waves, fee, ByteStr.empty, time.getTimestamp()))
  ...
  private def transferV2(sender: KeyPair, maxAmount: Long, time: Time) =
    (for {
      ...
      recipient <- accountGen
      fee       <- chooseNum(extraFee, (maxAmount * 0.1).toLong)
    } yield SignedTx.transfer(TxVersion.V2, sender, recipient.toAddress, Waves, amount, Waves, fee, ByteStr.empty,  time.getTimestamp())
      )
  ...
  private def transactionV1Gen(sender: KeyPair, ts: Long, feeAmount: Long): Gen[TransferTransaction] = accountGen.map { recipient =>
    SignedTx.transfer(1.toByte, sender, recipient.toAddress, Waves, waves(1), Waves, feeAmount, ByteStr.empty,  ts)
  }

  private def transactionV2Gen(sender: KeyPair, ts: Long, feeAmount: Long): Gen[TransferTransaction] = accountGen.map { recipient =>
    SignedTx.transfer(2.toByte, sender, recipient.toAddress, Waves, waves(1), Waves, feeAmount, ByteStr.empty,  ts)
  }
    ...
    "event stream" - {
      "fires events correctly" in {
        val preconditions = for {
          ...
          validTransfer   = SignedTx.transfer(TxVersion.V1, richAcc, secondAcc.toAddress, Waves, 1, Waves, fee, ByteStr.empty,  ts)
          invalidTransfer = SignedTx.transfer(TxVersion.V1, secondAcc, richAcc.toAddress, Waves, 2, Waves, fee, ByteStr.empty,  ts)
