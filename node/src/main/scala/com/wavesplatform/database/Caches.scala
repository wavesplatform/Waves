...
abstract class Caches extends Blockchain with Storage {
  ...
  private val objectWeigher = MemoryMeter.builder().build()

  private val scriptCache: LoadingCache[Address, Option[AccountScriptInfo]] =
    CacheBuilder
      .newBuilder()
      .maximumWeight(128 << 20)
      .weigher((_: Address, asi: Option[AccountScriptInfo]) => asi.map(s => objectWeigher.measureDeep(s).toInt).getOrElse(0))
      .recordStats()
