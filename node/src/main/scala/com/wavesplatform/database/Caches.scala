...
abstract class Caches(spendableBalanceChanged: Observer[(Address, Asset)]) extends Blockchain with ScorexLogging {
  ...
  protected def addressIdWithFallback(address: Address, newAddresses: Map[Address, Long]): Long =
  ...
  private[database] def addressId(address: Address): Option[Long] = addressIdCache.get(address)
  ...
  def append(diff: Diff, carryFee: Long, totalFee: Long, reward: Option[Long], htiSource: ByteStr, block: Block): Unit = {
    ...
    val addressTransactions: Map[Long, Seq[TransactionId]] =
      transactionList
        .flatMap {
          case (_, (tx, addrs)) =>
