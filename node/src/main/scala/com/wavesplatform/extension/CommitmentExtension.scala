...
class CommitmentExtension(context: Context) extends Extension with ScorexLogging {
  ...
  override def start(): Unit = {
    ...
    val attemptStart = if (context.blockchain.height == 0) {
      waitForNewHeight(0).map(_ => ())
    } else {
      Task.unit
    }
    ...
    attemptStart.map { _ =>
      ...
      if (initialHeight == 0) {
        // This case is handled by the initial wrapper, but keeping logic consistent just in case or for restart scenarios if needed,
        // though strictly speaking we are already > 0 here.
        // Actually, the initial wrapper guarantees we are > 0 if we reached here.
        // So we can simplify or just keep the logic.
        // Let's rely on the outer check.
         val h = context.blockchain.height
         val n = Math.floor((h - 1).toDouble / generationPeriodLength).toLong
         currentPeriod = ((n + 1) * generationPeriodLength).toInt
         heightLoop(h).runAsyncLogErr
      } else {
