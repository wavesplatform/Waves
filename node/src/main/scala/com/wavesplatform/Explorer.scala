...
import com.wavesplatform.state.{AddressId, Height, TransactionId}
    ...
    try {
      ...
      flag match {
        case "B" =>
          ...
          if (maybeBlockId.isDefined) {
            ...
            maybeBlockHeight.foreach { h =>
              val blockBytes = reader.blockBytes(h).get
        ...
        case "AA" =>
          ...
          val (txH, txN) = reader.loadTransactionHN(TransactionId @@ asset.id).get
        ...
        case "TXBH" =>
          ...
          val txs = reader.blockAt(h).get.transactionData
          println(txs.length)
          txs.foreach(println)
        ...
        case "APS" =>
          ...
          for {
            ...
            (txH, txN) <- reader.loadTransactionHN(TransactionId @@ asset.id).toSeq
            addressId <- db
