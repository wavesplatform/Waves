...
object BlockAppender extends ScorexLogging {
  ...
  def apply(
    ...
    log.info(s"Block received ${Instant.now()}")
    ...
    val handle = append.asyncBoundary.map {
      ...
      case Right(Some(_)) =>
        log.debug(s"${id(ch)} Appended $newBlock ${Instant.now()}")
      ...
      case Left(is: InvalidSignature) =>
        peerDatabase.blacklistAndClose(ch, s"Could not append $newBlock: $is ${Instant.now()}")

      case Left(ve) =>
        log.debug(s"${id(ch)} Could not append $newBlock generated by ${newBlock.header.generator}: $ve ${Instant.now()}")
