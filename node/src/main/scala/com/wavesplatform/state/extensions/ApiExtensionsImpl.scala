...
object ApiExtensionsImpl {
  def apply(b: Blockchain): ApiExtensions = b match {
    ...
    case upd: BlockchainUpdaterImpl =>
      ...
      val addressTransactions = new CompositeAddressTransactions(baseAddressTransactions, Height @@ upd.height, () => upd.bestLiquidDiff)
    ...
    case cb: CompositeBlockchain =>
      ...
      val addressTransactions = new CompositeAddressTransactions(baseAddressTransactions, Height @@ cb.height, () => cb.maybeDiff)
  ...
  private[this] def fromAddressTransactionsAndDistributions(at: AddressTransactions, d: Distributions): ApiExtensions = {
    new AddressTransactions with Distributions {
      ...
      override def assetDistributionAtHeight(asset: Asset.IssuedAsset, height: Int, count: Int, fromAddress: Option[Address]): Either[ValidationError, AssetDistributionPage] = d.assetDistributionAtHeight(asset, height, count, fromAddress)
      ...
      override def addressTransactionsObservable(address: Address, types: Set[TransactionParser], fromId: Option[ByteStr]): Observable[(Height, Transaction)] = at.addressTransactionsObservable(address, types, fromId)
    }
  }
}
