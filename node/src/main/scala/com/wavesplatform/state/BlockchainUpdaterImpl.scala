  ...
  override def removeAfter(blockId: ByteStr): Either[ValidationError, Seq[(Block, ByteStr)]] = writeLock {
    ...
    val result = if (prevNgState.exists(_.contains(blockId))) {
      ...
      blockchain
        .rollbackTo(blockId)
        .map { bs =>
          blockchainUpdateTriggers.onRollback(blockId, blockchain.height)
          bs ++ discardedNgBlock
        }
    ...
    ngState match {
      ...
      case Some(ng) =>
        ng.lastMicroBlock match {
          case None if ng.base.uniqueId != microBlock.prevResBlockRef =>
          ...
          case Some(prevMicro) if prevMicro.totalResBlockRef != microBlock.prevResBlockRef =>
          ...
          case _ =>
            for {
              ...
              totalSignatureValid <- ng
                .totalDiffOf(microBlock.prevResBlockRef)
                .toRight(GenericError(s"No referenced block exists: $microBlock"))
                .map {
                  case (accumulatedBlock, _, _, _, _) =>
                    Block
                      .create(accumulatedBlock, accumulatedBlock.transactionData ++ microBlock.transactionData, if (microBlock.version >= Block.ProtoBlockVersion) microBlock.totalSignature else microBlock.totalResBlockRef)
                      .signatureValid()
                }
              _ <- Either
                .cond(
              ...
              internalLastBlockInfo.onNext(LastBlockInfo(microBlock.totalResBlockRef, height, score, ready = true))
