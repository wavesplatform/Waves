...
class BlockchainUpdaterImpl(
  ...
  override def isLastBlockId(id: ByteStr): Boolean = readLock {
    ngState.exists(_.contains(id)) || lastBlock.exists(_.id() == id)
  }
  ...
  override def blockInfo(blockId: BlockId): Option[BlockInfo] = readLock {
    liquidBlockHeaderAndSize().filter(_.id() == blockId) orElse blockchain.blockInfo(blockId)
  }
    ...
    ngState
      ...
      .orElse(
        blockchain.lastBlock.map(
          b => BlockMinerInfo(NxtLikeConsensusBlockData(b.header.baseTarget, b.header.generationSignature), b.header.timestamp, b.id())
        )
      )
  }
