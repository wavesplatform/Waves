    ...
    accScriptEi match {
      case Right(Some(sc @ ContractScriptImpl(_, contract, _))) =>
        ...
        val contractFunc = tx.funcCallOpt match {
          case Some(fc) => contract.callableFuncs.find(_.u.name == functionName).flatMap(_ => tx.funcCallOpt)
          case None =>
            contract.callableFuncs
              .find(_.u.name == ContractEvaluator.DEFAULT_FUNC_NAME)
              .flatMap(_ => Some(FUNCTION_CALL(FunctionHeader.User(ContractEvaluator.DEFAULT_FUNC_NAME), List.empty)))
