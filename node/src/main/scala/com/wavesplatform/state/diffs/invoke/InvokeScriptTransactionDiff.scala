    ...
    accScriptEi match {
      case Right(Some(AccountScriptInfo(pk, ContractScriptImpl(version, contract), _, callableComplexities))) =>
        for {
          ...
          runsLimit = BigDecimal((feeInfo._1 - FeeConstants(InvokeScriptTransaction.typeId) * FeeUnit) / ScriptExtraFee).toIntExact
          _ = println((feeInfo, FeeConstants(InvokeScriptTransaction.typeId), FeeUnit, ScriptExtraFee))
          ...
          result <- for {
            scriptResult <- {
              ...
              TracedResult(
                scriptResultE,
                List(InvokeScriptTrace(tx.dAppAddressOrAlias, functionCall, scriptResultE.map(_._2._1), scriptResultE.fold(_.log, _._2._2)))
              )
            }
          ...
          } yield scriptResult._1 combine resultDiff
        } yield result
