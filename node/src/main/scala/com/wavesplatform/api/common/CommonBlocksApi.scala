...
object CommonBlocksApi {
  def apply(
      ...
      blockAt: Int => Option[(BlockMeta, Seq[(Transaction, Boolean)])]
    ...
    def blockAtHeight(height: Int): Option[(BlockMeta, Seq[(Transaction, Boolean)])] = blockAt(height)
    ...
    def block(blockId: BlockId): Option[(BlockMeta, Seq[(Transaction, Boolean)])] = heightOf(blockId).flatMap(h => blockAt(h))
  }
}
