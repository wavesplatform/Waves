...
object UtilsEvaluator {
  ...
  def executeExpression2(
      ...
      thisFuncCall: Terms.FUNCTION_CALL = Terms.FUNCTION_CALL(FunctionHeader.User("foo"), List(CONST_LONG(1)))
    ...
    for {
      ...
      invoke = new InvokeScriptTransactionLike {
        ...
        override def funcCall: Terms.FUNCTION_CALL = thisFuncCall
        ...
        override def assetFee: (Asset, Long) = Asset.Waves -> 500000 // TODO
        ...
        override def checkedAssets: Seq[IssuedAsset] = scriptedAssets

        override val tpe: TransactionType = TransactionType.InvokeScript
      }
      result = InvokeScriptTransactionDiff(
        blockchain,
        System.currentTimeMillis(), // blockTime
        limitedExecution = false
      )(invoke)
      result <- result.resultE.map { all =>
        result.trace
          .collect {
            case x: InvokeScriptTrace =>
              x.resultE.map(x => (x.returnedValue, all))
          }
          .mkString("\n")
      }
    } yield result
