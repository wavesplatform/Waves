...
final case class TransactionJsonSerializer(blockchain: Blockchain, commonApi: CommonTransactionsApi) {
  ...
  def leaseRefCodec(numbersAsString: Boolean): JsonValueCodec[LeaseRef] = new OnlyEncodeJsonValueCodec[LeaseRef] {
    override def encodeValue(l: LeaseRef, out: JsonWriter): Unit = {
      ...
      out.writeKeyValue("originTransactionId", l.originTransactionId.toString)
      out.writeKeyValue("sender", l.sender.toString)
      out.writeKeyValue("recipient", l.recipient.toString)
      out.writeKeyValue("amount", l.amount, numbersAsString)
      out.writeKeyValue("height", l.height, numbersAsString)
