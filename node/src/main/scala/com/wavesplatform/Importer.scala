        ...
        createInputStream(filename) match {
          case Success(inputStream) =>
            ...
            val blockchainUpdatesScheduler: Option[Scheduler] =
              if (settings.enableBlockchainUpdates) Some(Scheduler.singleThread("blockchain-updates")) else None
            val blockchainUpdated = blockchainUpdatesScheduler map (ConcurrentSubject.publish[BlockchainUpdated](_))
            val blockchainUpdater = StorageFactory(settings, db, time, Observer.empty(UncaughtExceptionReporter.LogExceptionsToStandardErr), false, blockchainUpdated)
