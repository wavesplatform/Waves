...
object Merkle {
  ...
  def calcTransactionProof(allTransactions: Seq[Transaction], transaction: Transaction): Option[TransactionProof] =
    for {
      proof               <- mkMerkleTree(allTransactions).proofByElement(mkMerkleLeaf(transaction))
      (_, transactionIdx) <- allTransactions.zipWithIndex.find { case (tx, _) => tx.id() == transaction.id() }
