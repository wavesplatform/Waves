...
object Address extends ScorexLogging {
  ...
  def fromString(addressStr: String, checkChainId: Boolean = true): Either[ValidationError, Address] = {
    ...
    for {
      ...
      address   <- fromBytes(byteArray, if (checkChainId) scheme.chainId else byteArray(1))
    } yield address
  }
