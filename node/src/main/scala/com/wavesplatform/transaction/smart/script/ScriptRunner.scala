...
import com.wavesplatform.features.EvaluatorModeProvider._
    ...
    def evaluate(ctx: EvaluationContext[Environment, Id], expr: EXPR): (Log[Id], Int, Either[ExecutionError, EVALUATED]) = {
      val (log, unusedComplexity, result) =
        if (complexityLimit == Int.MaxValue)
          EvaluatorV2.applyCompleted(ctx, expr, script.stdLibVersion, blockchain.newEvaluatorMode)
        else
          EvaluatorV2.applyOrDefault(ctx, expr, script.stdLibVersion, complexityLimit, blockchain.newEvaluatorMode, _ => Right(default))
