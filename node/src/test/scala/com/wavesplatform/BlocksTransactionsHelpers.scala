...
trait BlocksTransactionsHelpers { self: TransactionGen =>
  object QuickTX {
    ...
    def invokeScript(
      ...
      for {
        timestamp <- timestamp
      } yield InvokeScriptTransaction.selfSigned(1.toByte, from, dapp, Some(call), payments, FeeAmount * 2, Waves, extraFeePerStep = InvokeScriptTransaction.DefaultExtraFeePerStep, timestamp = timestamp).explicitGet()
  }
