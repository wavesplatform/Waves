    ...
    scenario(sponsorPreconditions, SponsoredActivatedAt0WavesSettings) {
      case (domain, (genesis, masterToAlice, feeAsset, sponsor, aliceToBob, bobToMaster, _)) =>
...

        {
          ...
          domain.blockchainUpdater.bestLiquidDiffAndFees.map(_._4) should contain(block0TotalFee)
        }

        {
          ...
          domain.blockchainUpdater.bestLiquidDiffAndFees.map(_._4) should contain(block0TotalFee + microBlocksWavesFee)
        }
    }
  }
