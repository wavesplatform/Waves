...
class UtxPoolSpecification
  ...
  private val withBlacklisted = (for {
    ...
    val settings =
      UtxSettings(10, PoolDefaultMaxBytes, 1000, Set(sender.addressString), Set.empty, allowTransactionsFromSmartAccounts = true, allowSkipChecks = false)
    val utxPool = new UtxPoolImpl(time, bcu, ignoreSpendableBalanceChanged, settings)
    (sender, utxPool, txs)
  ...
  private val withBlacklistedAndAllowedByRule = (for {
    ...
    val settings =
      UtxSettings(txs.length,
                  PoolDefaultMaxBytes,
                  1000,
                  Set(sender.addressString),
                  Set(recipient.addressString),
  ...
  private def massTransferWithBlacklisted(allowRecipients: Boolean) =
    (for {
      ...
      val whitelist: Set[String] = if (allowRecipients) recipients.map(_.addressString).toSet else Set.empty
      val settings =
        UtxSettings(txs.length,
                    PoolDefaultMaxBytes,
                    1000,
