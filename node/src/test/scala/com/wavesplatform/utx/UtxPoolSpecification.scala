...
class UtxPoolSpecification
  ...
  private def transfer(sender: KeyPair, maxAmount: Long, time: Time) =
    (for {
      ...
      .selfSigned(1.toByte, sender, recipient, Waves, amount, Waves, fee, None, time.getTimestamp())
      .explicitGet())
  ...
  private def transferV2(sender: KeyPair, maxAmount: Long, time: Time) =
    (for {
      ...
      recipient <- accountGen
      fee       <- chooseNum(extraFee, (maxAmount * 0.1).toLong)
    } yield TransferTransaction.selfSigned(TxVersion.V2, sender, recipient, Waves, amount, Waves, fee, None, time.getTimestamp()).explicitGet())
  ...
  private val withValidPayments = (for {
    ...
    val settings =
      UtxSettings(10, PoolDefaultMaxBytes, 1000, Set.empty, Set.empty, allowTransactionsFromSmartAccounts = true, allowSkipChecks = false)
    val utxPool = new UtxPoolImpl(time, bcu.blockchain, ignoreSpendableBalanceChanged, settings, enablePriorityPool = true)
    ...
    forAll(stateGen, chooseNum(2, txCount).label("txCount")) {
      case ((sender, senderBalance, bcu), count) =>
        ...
        forAll(listOfN(count, transfer(sender, senderBalance / 2, time))) { txs =>
          val utx = new UtxPoolImpl(time, bcu.blockchain, ignoreSpendableBalanceChanged, utxSettings, enablePriorityPool = true)
          f(txs, utx, time)
        }
    }
  ...
  private def withScriptedAccount(scEnabled: Boolean): Gen[(KeyPair, Long, UtxPoolImpl, Long)] =
    for {
      (sender, senderBalance, bcu) <- stateGen
      time          = new TestTime()
      preconditions = preconditionBlocks(bcu.blockchain.lastBlockId.get, sender, time)
    } yield {
      ...
      (sender, senderBalance, utx, bcu.blockchain.lastBlockTimestamp.getOrElse(0L))
    }

  private def transactionV1Gen(sender: KeyPair, ts: Long, feeAmount: Long): Gen[TransferTransaction] = accountGen.map { recipient =>
    TransferTransaction.selfSigned(1.toByte, sender, recipient, Waves, waves(1), Waves, feeAmount, None, ts).explicitGet()
  }

  private def transactionV2Gen(sender: KeyPair, ts: Long, feeAmount: Long): Gen[TransferTransaction] = accountGen.map { recipient =>
    TransferTransaction.selfSigned(2.toByte, sender, recipient, Waves, waves(1), Waves, feeAmount, None, ts).explicitGet()
  }
      ...
      "when pack time limit is exceeded" - {
        "always packs the first transaction" in forAll(stateWithTransfer) {
          case (time, bcu, transfer) =>
            ...
            val settings =
              UtxSettings(10, PoolDefaultMaxBytes, 1000, Set.empty, Set.empty, allowTransactionsFromSmartAccounts = true, allowSkipChecks = false)
            val utxPool = new UtxPoolImpl(time, bcu.blockchain, ignoreSpendableBalanceChanged, settings, true, nanoTimeSource = () => nanoTimeSource())
      ...
      "applies chains correctly" in forAll(genChain) {
        case (genBlock, (block1, mbs1), (block2, mbs2), block3, block4) =>
          withDomain(settingsWithNG) { d =>
            ...
            (pos.validateGenerationSignature _).when(*).returning(Right(new Array[Byte](32)))
            ...
            val microBlockAppender = MicroblockAppender(d.blockchainUpdater, utx, scheduler) _
        ...
        forAll(preconditions) {
          case (genesis, validTransfer, invalidTransfer) =>
            withDomain() { d =>
              ...
              val differ = TransactionDiffer(d.blockchain.lastBlockTimestamp, System.currentTimeMillis(), verify = false)(
                d.blockchain,
                _: Transaction
              ).resultE.explicitGet()
