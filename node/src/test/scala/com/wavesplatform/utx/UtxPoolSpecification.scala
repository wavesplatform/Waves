  ...
  "UTX Pool" - {
    ...
    "packUnconfirmed result is limited by constraint" in forAll(dualTxGen) {
      case (utx, _, txs, _) =>
        ...
        val (packed, restUpdated) = utx.packUnconfirmed(rest, 5.seconds, Duration.Zero)
    ...
    "evicts expired transactions when packUnconfirmed is called" in forAll(dualTxGen) {
      case (utx, time, txs, _) =>
        ...
        val (packed, _) = utx.packUnconfirmed(limitByNumber(100), 5.seconds, Duration.Zero)
    ...
    "correctly process constraints in packUnconfirmed" in {
      withDomain(wavesplatform.history.TransfersV2ActivatedAt0WavesSettings) { d =>
        ...
        val (packed, _) = utx.packUnconfirmed(constraint, 5.seconds, Duration.Zero)
    ...
    "pessimisticPortfolio" - {
      ...
      "is changed after transactions with these assets are removed" in forAll(withValidPayments) {
        case (sender, _, utxPool, time, _) =>
          ...
          utxPool.packUnconfirmed(limitByNumber(100), 5.seconds, Duration.Zero)
      ...
      "doesnt validate transactions which are removed" in {
        ...
        forAll(gen) {
          case (tx1, rest) =>
            ...
            utx.packUnconfirmed(MultiDimensionalMiningConstraint.unlimited, Duration.Inf, Duration.Zero) should matchPattern {
    ...
    "priority pool" - {
      ...
      "preserves correct order of transactions" in forAll(gen) {
        case (tx1, nonScripted, scripted) =>
          ...
          utx
            .packUnconfirmed(MultiDimensionalMiningConstraint(NonEmptyList.of(OneDimensionalMiningConstraint(1, TxEstimators.one, ""))), Duration.Inf, Duration.Zero)
            ._1 shouldBe Some(minedTxs.head +: Nil)
          val expectedTxs = minedTxs :+ tx1
          utx.packUnconfirmed(MultiDimensionalMiningConstraint.unlimited, Duration.Inf, Duration.Zero)._1 shouldBe Some(expectedTxs)
          ...
          utx.packUnconfirmed(MultiDimensionalMiningConstraint.unlimited, Duration.Inf, Duration.Zero)._1 shouldBe Some(expectedTxs1)
          ...
          utx.removeAll(expectedTxs2)
          utx.all shouldBe empty
          utx.packUnconfirmed(MultiDimensionalMiningConstraint.unlimited, Duration.Inf, Duration.Zero)._1 shouldBe empty
      ...
      "removes priority transactions from ordinary pool on pack" in forAll(gen) {
        case (_, nonScripted, scripted) =>
          ...
          utx.packUnconfirmed(MultiDimensionalMiningConstraint.unlimited, Duration.Inf, Duration.Zero)._1 shouldBe Some(nonScripted)
          utx.invokePrivate(nonPriorityTransactions()) shouldBe empty
      }
      ...
      "takes into account priority txs when pack" in forAll(genDependent) {
        case (tx1, tx2) =>
          ...
          utx.packUnconfirmed(MultiDimensionalMiningConstraint.unlimited, Duration.Inf, Duration.Zero)._1 shouldBe Some(tx1 :: tx2 :: Nil)
      }
      ...
      "applies chains correctly" in forAll(genChain) {
        case (genBlock, (block1, mbs1), (block2, mbs2), block3, block4) =>
          withDomain(settingsWithNG) { d =>
            ...
            utx.packUnconfirmed(MultiDimensionalMiningConstraint.unlimited, Duration.Inf, Duration.Zero)._1 shouldBe Some(mbs1.last.transactionData)
            ...
            utx.packUnconfirmed(MultiDimensionalMiningConstraint.unlimited, Duration.Inf, Duration.Zero)._1 shouldBe Some(expectedTxs2)
          }
      }
    }
        ...
        forAll(preconditions) {
          case (genesis, validTransfer, invalidTransfer) =>
            withDomain() { d =>
              ...
              utxPool.packUnconfirmed(MultiDimensionalMiningConstraint.unlimited, Duration.Inf, Duration.Zero)
              assertEvents {
