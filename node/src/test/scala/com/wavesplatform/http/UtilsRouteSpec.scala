...
class UtilsRouteSpec extends RouteSpec("/utils") with RestAPISettingsHelper with PropertyChecks with PathMockFactory with Inside with WithDomain {
  ...
  routePath("/script/evaluate/{address}") in withDomain(DomainPresets.RideV6) { d =>
    ...
    evalScript("testCallable()") ~> route ~> check {
      responseAs[JsValue] should matchJson(
        """{"result":{"type":"Array","value":[{"type":"BinaryEntry","value":{"key":{"type":"String","value":"test"},"value":{"type":"ByteVector","value":"11111111111111111111111111"}}}]},"complexity":2,"expr":"testCallable()","address":"3MtGzgmNa5fMjGCcPi5nqMTdtZkfojyWHL9"}"""
      )
    }
    ...
    evalScript("testNone()") ~> route ~> check {
      responseJson shouldBe Json.obj(
        "error"   -> 306,
        "message" -> "ScriptExecutionError(error = Function or type 'testNone' not found, type = Account, log = \n\ttestNone.@args = []\n)"
      )
    }
    ...
    evalScript("testF()") ~> route ~> check {
      responseJson shouldBe Json.obj(
        "error" -> 306,
        "message" -> "ScriptExecutionError(error = Test, type = Account, log = \n\ttestF.@args = []\n\tthrow.@args = [\n\t\t\"Test\"\n\t]\n\tthrow.@complexity = 1\n\t@complexityLimit = 25999\n)"
      )
    }
