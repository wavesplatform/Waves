...
class BlockV5Test
  ...
  private val updaterScenario = for {
    ...
    (b5, m5s) = chainBaseAndMicro(m4s.head.totalResBlockRef, Seq.empty, Seq(Seq(tx2)), miner1, Block.RewardBlockVersion, ntpNow)
    (b6, m6s) = chainBaseAndMicro(m5s.head.totalResBlockRef, Seq(tx3), Seq(Seq(tx4)), miner2, Block.ProtoBlockVersion, ntpNow)
    (b7, m7s) = chainBaseAndMicro(m6s.head.totalResBlockRef, Seq.empty, Seq(Seq(tx5)), miner1, Block.ProtoBlockVersion, ntpNow)
      ...
      case (acc, genesis) =>
        ...
        withDomain(WavesSettings.default().copy(blockchainSettings = WavesSettings.default().blockchainSettings.copy(functionalitySettings = fs))) {
          d =>
            ...
            val mb1 = d.blockchainUpdater.microBlock(microBlocks.head.totalResBlockRef).get
            mb1.totalSignature should have length crypto.SignatureLength
            mb1.prevResBlockRef should not be keyBlock.signature
            mb1.prevResBlockRef shouldBe keyBlock.uniqueId
            mb1.totalResBlockRef should have length crypto.DigestLength
        }
    }
  }
