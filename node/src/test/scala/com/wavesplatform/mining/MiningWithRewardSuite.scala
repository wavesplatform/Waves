  ...
  it should "generate valid blocks with transactions of version 4" in {
    val bps: Seq[BlockProducer] = Seq(
      (ts, reference, account) => {
        ...
        val tx1 = TransferTransaction.selfSigned(2.toByte, ts, account, recipient1, Waves, 10 * Constants.UnitsInWave, Waves, 400000, Array())
          .explicitGet()
        val tx2 = TransferTransaction.selfSigned(2.toByte, ts, account, recipient2, Waves, 5 * Constants.UnitsInWave, Waves, 400000, Array())
          .explicitGet()
    ...
    val txs: Seq[TransactionProducer] = Seq(
      (ts, account) => {
        val recipient1 = createAccount.toAddress
        TransferTransaction.selfSigned(2.toByte, ts, account, recipient1, Waves, 10 * Constants.UnitsInWave, Waves, 400000, Array())
          .explicitGet()
      }
    )
