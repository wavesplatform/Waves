...
trait RequestGen extends TransactionGen { _: Suite =>
  ...
  } yield SignedCreateAliasV1Request(_alias.miner.toString, _alias.fee, _alias.alias.name, _timestamp, _signature)

  val leaseReq: G[SignedLeaseV1Request] = for {
    _signature <- signatureGen
    _timestamp <- ntpTimestampGen
    _alias     <- leaseGen
  } yield SignedLeaseV1Request(_alias.miner.toString, _alias.amount, _alias.fee, _alias.recipient.toString, _timestamp, _signature)

  val leaseCancelReq: G[SignedLeaseCancelV1Request] = for {
    _signature <- signatureGen
    _timestamp <- ntpTimestampGen
    _cancel    <- leaseCancelGen
  } yield SignedLeaseCancelV1Request(_cancel.miner.toString, _cancel.leaseId.toString, _timestamp, _signature, _cancel.fee)
}
