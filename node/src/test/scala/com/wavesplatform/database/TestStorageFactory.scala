...
object TestStorageFactory {
  ...
  def apply(
    ...
    val levelDBWriter: LevelDBWriter = new LevelDBWriter(db, settings.blockchainSettings, settings.dbSettings) {
    ...
    (
      new BlockchainUpdaterImpl(levelDBWriter, settings, time, blockchainUpdateTriggers, loadActiveLeases(db, _, _)),
      levelDBWriter
    )
  }
}
