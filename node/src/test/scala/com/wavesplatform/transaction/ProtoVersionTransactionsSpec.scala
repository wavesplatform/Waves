...
class ProtoVersionTransactionsSpec extends FreeSpec with TransactionGen with Matchers {
  ...
  "all txs" - {
    "CreateAliasTransaction" in {
      ...
      val base64Tx = PBUtils.encodeDeterministic(PBTransactions.protobuf(aliasTx))
    ...
    "IssueTransaction/ReissueTransaction/BurnTransaction" in {
      ...
      val issueTx = IssueTransaction
        .selfSigned(TxVersion.V3, Account, name, description, quantity, decimals, reissuable, script = None, MinIssueFee, Now)
        .explicitGet()
      val base64IssueStr = PBUtils.encodeDeterministic(PBTransactions.protobuf(issueTx))
      ...
      val reissueTx = ReissueTransaction
        ...
        .explicitGet()
      val base64reissueStr = PBUtils.encodeDeterministic(PBTransactions.protobuf(reissueTx))
      ...
      val base64BurnStr = PBUtils.encodeDeterministic(PBTransactions.protobuf(burnTx))
    ...
    "DataTransaction" in {
      ...
      val base64Str = PBUtils.encodeDeterministic(PBTransactions.protobuf(dataTx))
    ...
    "ExchangeTransaction" in {
      ...
      val base64Str = PBUtils.encodeDeterministic(PBTransactions.protobuf(exchangeTx))
    ...
    "InvokeScriptTransaction" in {
      ...
      val invokeScriptTx = InvokeScriptTransaction
        ...
        .explicitGet()
      val base64Str = PBUtils.encodeDeterministic(PBTransactions.protobuf(invokeScriptTx))
    ...
    "LeaseTransaction/LeaseCancelTransaction" in {
      ...
      val leaseCancelTx        = LeaseCancelTransaction.signed(TxVersion.V3, Account.publicKey, leaseTx.id(), MinFee, Now, Account.privateKey).explicitGet()
      val base64LeaseStr       = PBUtils.encodeDeterministic(PBTransactions.protobuf(leaseTx))
      val base64CancelLeaseStr = PBUtils.encodeDeterministic(PBTransactions.protobuf(leaseCancelTx))
    ...
    "TransferTransaction" in {
      ...
      val base64Str = PBUtils.encodeDeterministic(PBTransactions.protobuf(transferTx))
    ...
    "MassTransferTransaction" in {
      ...
      val base64Str = PBUtils.encodeDeterministic(PBTransactions.protobuf(massTransferTx))
    ...
    "SetScriptTransaction" in {
      ...
      val base64Str   = PBUtils.encodeDeterministic(PBTransactions.protobuf(setScriptTx))
    ...
    "SetAssetScriptTransaction" in {
      ...
      val base64Str        = PBUtils.encodeDeterministic(PBTransactions.protobuf(setAssetScriptTx))
    ...
    "SponsorshipTransaction" in {
      ...
      val base64Str     = PBUtils.encodeDeterministic(PBTransactions.protobuf(sponsorshipTx))

      decode(base64Str) shouldBe sponsorshipTx
    }

    def decode(bytes: Array[Byte]): Transaction = {
      PBTransactions.vanilla(PBSignedTransaction.parseFrom(bytes)).explicitGet()
    }
  }
}
