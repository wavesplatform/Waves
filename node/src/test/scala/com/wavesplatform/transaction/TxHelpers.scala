...
object TxHelpers {
  ...
  def invoke(dApp: AddressOrAlias, func: String, args: Seq[EXPR] = Nil, payments: Seq[Payment] = Nil): InvokeScriptTransaction = {
    ...
    InvokeScriptTransaction.selfSigned(TxVersion.V1, defaultSigner, dApp, Some(fc), payments, TestValues.fee, Asset.Waves, feeIncreaseFactor = 100, timestamp).explicitGet()
  }
}
