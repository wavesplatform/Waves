  ...
  property("can resolve best liquid block") {
    forAll(preconditionsAndPayments(5)) {
      case (genesis, payments) =>
        ...
        ng.bestLiquidBlock.header.uniqueId shouldBe microBlocks.last.totalResBlockSig

        new NgState(block, Diff.empty, 0L, 0L, Set.empty, Some(0), Map.empty).bestLiquidBlock.header.uniqueId shouldBe block.header.uniqueId
    }
  }
  ...
  property("can resolve best last block") {
    forAll(preconditionsAndPayments(5)) {
      case (genesis, payments) =>
        ...
        ng.bestLastBlockInfo(0).blockId shouldBe block.header.uniqueId
  ...
  property("calculates carry fee correctly") {
    forAll(preconditionsAndPayments(5)) {
      case (genesis, payments) =>
        ...
        ng.totalDiffOf(block.header.uniqueId).map(_._3) shouldBe Some(0L)
