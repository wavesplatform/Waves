    ...
    def assert(settings: WavesSettings, fixed: Boolean) =
      withDomain(settings) { d =>
        ...
        d.blockchain.balanceSnapshots(defaultAddress, 1, None) shouldBe List(
          BalanceSnapshot(0, 600000000, 0, 0, false)
        )
        ...
        d.blockchain.balanceSnapshots(defaultAddress, 1, None) shouldBe (
          if (fixed)
            List(
              BalanceSnapshot(1, 1199999999, 0, 0, false),
              BalanceSnapshot(1, 599399999, 0, 0, false)
            )
          else
            List(BalanceSnapshot(1, 1199999999, 0, 0, false))
        )
        ...
        d.appendKeyBlock()
        d.blockchain.balanceSnapshots(defaultAddress, 1, None) shouldBe List(
          BalanceSnapshot(2, 1799999998, 0, 0, false),
        ...
        d.appendKeyBlock()
        d.blockchain.balanceSnapshots(defaultAddress, 1, None) shouldBe List(
          BalanceSnapshot(3, 2399999997L, 0, 0, false),
