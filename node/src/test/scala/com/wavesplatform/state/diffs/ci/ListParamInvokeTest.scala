  ...
  private def paymentPreconditions(dApp: Script): Gen[(List[GenesisTransaction], SetScriptTransaction, InvokeScriptTransaction, Address)] =
    for {
      ...
      for {
        ...
        ci       <- InvokeScriptTransaction.selfSigned(1.toByte, invoker, master.toAddress, functionCall, Nil, fee, Waves, extraFeePerStep = InvokeScriptTransaction.DefaultExtraFeePerStep, ts + 3)
