      ...
      case (version, masterAcc, recipient, genesis, dataTransaction, transferTx) =>
        val fs = smartEnabledFS.copy(
          preActivatedFeatures = smartEnabledFS.preActivatedFeatures ++
            Map(BlockchainFeatures.ContinuationTransaction.id -> 0)
        )
        assertDiffAndState(fs) { append =>
          ...
          val compiledScript = ContractScript(version, compiler.ContractCompiler(ctx.compilerContext, expr, version).explicitGet()).explicitGet()
          val setScriptTx =
