  ...
  property("check pre-Ride4DApps scripts run count") {
    forAll(for {
      ...
      tr2 = TransferTransaction
        ...
        .explicitGet()
      mt1 = MassTransferTransaction.selfSigned(1.toByte, Waves, master, List(ParsedTransfer(acc, 1)), timestamp, fee, ByteStr(Array())).explicitGet()
      mt2 = MassTransferTransaction
        .selfSigned(1.toByte, IssuedAsset(issueScr.id()), master, List(ParsedTransfer(acc, 1)), timestamp, fee, ByteStr(Array()))
        .explicitGet()
