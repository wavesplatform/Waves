...
class InvokeExpressionTest extends PropSpec with ScalaCheckPropertyChecks with TransactionGen with WithDomain with EitherValues {
  ...
  property("activation") {
    ...
    withDomain(RideV5) { d =>
      d.appendBlock(genesisTxs: _*)
      intercept[Exception](d.appendBlock(invoke)).getMessage should include("Ride V6, MetaMask support, Invoke Expression feature has not been activated yet")
    }
  }
  ...
  property("complexity limit") {
    val (genesisTxs, invoke) = scenario(sigVerifyCount = 55)
    withDomain(RideV6) { d =>
      d.appendBlock(genesisTxs: _*)
      intercept[Exception](d.appendBlock(invoke)).getMessage should include("Contract function (default) is too complex: 10348 > 10000")
    }
  }
