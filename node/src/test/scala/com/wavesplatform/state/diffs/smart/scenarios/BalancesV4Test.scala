...
class BalancesV4Test extends PropSpec with PropertyChecks with WithState with TransactionGen with NoShrink {
  ...
  private val preconditionsAndTransfer = for {
    ...
    ci        = InvokeScriptTransaction.selfSigned(1.toByte, master, dapp.toAddress, functionCall, Nil, InvokeScriptTxFee, Waves, feeIncreaseFactor = 1, ts + 3).explicitGet()
    ...
    forAll(for { a <- accountGen; b <- accountGen } yield (a, b)) {
      case (acc1, acc2) =>
        ...
        val setScript = SetScriptTransaction
          ...
          .explicitGet()
        val ci = InvokeScriptTransaction.selfSigned(1.toByte, acc1, acc1.toAddress, functionCall, Nil, InvokeScriptTxFee, Waves, feeIncreaseFactor = 1, nextTs).explicitGet()
    ...
    forAll(for { a <- accountGen; b <- accountGen } yield (a, b)) {
      case (acc1, acc2) =>
        ...
        val setScript = SetScriptTransaction
          .selfSigned(1.toByte, acc1, Some(dappScript(ByteStr(acc2.toAddress.bytes), issue.id())), SetScriptFee, nextTs)
          .explicitGet()
        val ci = InvokeScriptTransaction.selfSigned(1.toByte, acc2, acc1.toAddress, functionCall, Nil, InvokeScriptTxFee, Waves, feeIncreaseFactor = 1, nextTs).explicitGet()
