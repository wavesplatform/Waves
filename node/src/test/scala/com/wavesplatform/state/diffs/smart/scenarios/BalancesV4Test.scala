...
class BalancesV4Test extends PropSpec with WithState {
  ...
  property("Waves balance details") {
    ...
    assertDiffAndState(
      ...
      val useLiquidDiff = new UseLiquidDiff { override def apply[A](f: Option[(Height, Diff)] => A): A = f(Some((Height(1), d))) }
      val apiBalance    = com.wavesplatform.api.common.CommonAccountsApi(useLiquidDiff, db, s).balanceDetails(acc1.toAddress).explicitGet()
