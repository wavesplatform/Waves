  ...
  property("disallows other network") {
    val preconditionsAndPayment: Gen[(GenesisTransaction, Transaction)] = for {
      ...
      tx <- Gen.oneOf(
        ...
        InvokeScriptTransaction(TxVersion.V2, master.publicKey, invChainAddrOrAlias, None, Nil, amount, Waves, feeIncreaseFactor = 100, timestamp, Proofs.empty, invChainId)
          .signWith(master.privateKey),
