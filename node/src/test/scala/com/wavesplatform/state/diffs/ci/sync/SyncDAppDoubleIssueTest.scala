...
class SyncDAppDoubleIssueTest extends PropSpec with WithDomain with TransactionGenBase {
  ...
  property("issue the same asset via 2 dApps") {
    for {
      ...
      withDomain(domainSettingsWithFS(settings)) { d =>
        ...
        d.liquidDiff.errorMessage(invoke.txId).get.text should include("already issued")
      }
    }
  }
}
