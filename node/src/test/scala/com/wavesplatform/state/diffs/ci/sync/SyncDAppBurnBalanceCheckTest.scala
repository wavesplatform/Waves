...
class SyncDAppBurnBalanceCheckTest extends PropSpec with WithDomain {
  ...
  property("negative balance always rejects tx after syncDAppCheckTransfersHeight") {
    for {
      ...
      val preparingTxs = genesis ++ Seq(issue, transfer, setScript1, setScript2)
      ...
      withDomain(domainSettingsWithFS(settings)) { d =>
        ...
        d.appendBlock()
        d.appendBlockE(invoke2) should produce(s"Sync call leads to temporary negative asset $asset balance = -100 for address ${dApp2.toAddress}")
      }
    }
  }
}
