...
class InvokeScriptTransactionDiffTest
  ...
  property("payment should be positive") {
    forAll(for {
      ...
      ci = InvokeScriptTransaction.selfSigned(1.toByte, invoker, master.toAddress, Some(fc), Seq(Payment(-1, Waves)), fee, Waves, feeIncreaseFactor = InvokeScriptTransaction.DefaultFeeIncreaseFactor, ts)
    ...
    val uniqueAssetIdScenario =
      for {
        ...
        invokeTx = InvokeScriptTransaction
          .selfSigned(TxVersion.V2, invoker, master.toAddress, Some(fc), Seq(), fee, Waves, feeIncreaseFactor = InvokeScriptTransaction.DefaultFeeIncreaseFactor, ts + 3)
          .explicitGet()
  ...
  property(s"accepts failed transactions after ${BlockchainFeatures.BlockV5} activation") {
    ...
    val failedTxScenario =
      for {
        ...
        invokeTx = InvokeScriptTransaction
          .selfSigned(TxVersion.V2, invoker, master.toAddress, Some(fc), Seq(), fee, feeAsset, feeIncreaseFactor = InvokeScriptTransaction.DefaultFeeIncreaseFactor, ts + 3)
          .explicitGet()
        ...
        txs = Seq("throw", "insufficient fee", "negative amount", "overflow amount", "self payment", "max actions", "invalid data entries", "ok")
          .map { arg =>
            ...
            InvokeScriptTransaction
              .selfSigned(TxVersion.V2, invoker, master.toAddress, Some(fc), Seq(), fee, Waves, feeIncreaseFactor = InvokeScriptTransaction.DefaultFeeIncreaseFactor, ts + 4)
              .explicitGet()
          }
  ...
  property("counts complexity correctly for failed transactions (asset script fails)") {
    ...
    val scenario =
      for {
        ...
        invokeTx = InvokeScriptTransaction
          .selfSigned(TxVersion.V3, invoker, master.toAddress, Some(fc), payments, fee, Waves, feeIncreaseFactor = InvokeScriptTransaction.DefaultFeeIncreaseFactor, ts + 6)
          .explicitGet()
