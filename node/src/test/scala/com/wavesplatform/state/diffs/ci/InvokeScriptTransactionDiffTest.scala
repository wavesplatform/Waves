  ...
  property("Contract payment should be positive") {
    forAll(for {
      ...
      case (acc, amount, genesis, setScript, ci, asset, master, ts) =>
        val t =
          TransferTransaction
            .selfSigned(2.toByte, ts, master, acc, IssuedAsset(asset.id()), asset.quantity / 10, Waves, enoughFee, Array[Byte]())
            .explicitGet()
  ...
  property("invoking contract with sponsored fee") {
    forAll(for {
      ...
      case (ts, acc, amount, genesis, setScript, ci, master, sponsoredAsset, setSponsorship) =>
        val t =
          TransferTransaction
            .selfSigned(2.toByte, ts, master, ci.sender, IssuedAsset(sponsoredAsset.id()), sponsoredAsset.quantity / 10, Waves, enoughFee, Array[Byte]())
            .explicitGet()
        assertDiffAndState(
