...
class InvokeScriptTransactionDiffTest
      ...
      ci = InvokeScriptTransaction
        .selfSigned(
          ...
          if (sponsored) sponsoredFee else
            fee
          ,
          if (sponsored) sponsorTx.assetId else
            Waves
          ,
          ts + 3
        )
  ...
  val chainId: TxVersion     = ChainId.global
  val enoughFee: Long = FeeValidation.ScriptExtraFee + FeeValidation.FeeConstants(IssueTransaction.typeId) * FeeValidation.FeeUnit
    ...
    forAll(for {
      ...
      case (genesis, setScript, issue, sponsorFee, invoke) =>
        assertDiffEi(Seq(TestBlock.create(genesis ++ Seq(issue, sponsorFee, setScript))), TestBlock.create(Seq(invoke)), fs) { diff =>
          invoke.feeAssetId shouldBe sponsorFee.assetId
          invoke.dAppAddressOrAlias shouldBe invoke.sender.toAddress

          val dv           = diff.explicitGet()
          val senderChange = dv.portfolios(invoke.sender).balanceOf(sponsorFee.assetId)
