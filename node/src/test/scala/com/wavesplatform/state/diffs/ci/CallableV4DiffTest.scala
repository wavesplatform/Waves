    ...
    val deleteEntryPreconditions: Gen[(List[GenesisTransaction], SetScriptTransaction, InvokeScriptTransaction, KeyPair)] =
      for {
        ...
        for {
          ...
          ci       <- InvokeScriptTransaction.selfSigned(1.toByte, invoker, master.toAddress, None, Nil, fee, Waves, feeIncreaseFactor = InvokeScriptTransaction.DefaultFeeIncreaseFactor, ts + 3)
  ...
  private def multiActionPreconditions(
    ...
    for {
      ...
      for {
        ...
        ci       <- InvokeScriptTransaction.selfSigned(TxVersion.V1, invoker, master.toAddress, None, Nil, fee, Waves, feeIncreaseFactor = InvokeScriptTransaction.DefaultFeeIncreaseFactor, ts + 3)
