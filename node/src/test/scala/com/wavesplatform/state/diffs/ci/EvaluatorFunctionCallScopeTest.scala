...
class EvaluatorFunctionCallScopeTest extends PropSpec with WithDomain with TransactionGenBase {
  ...
  private def settings(fix: Boolean) =
    if (fix)
      TestFunctionalitySettings.withFeatures(BlockV5, SynchronousCalls, RideV6)
    else
      TestFunctionalitySettings.withFeatures(BlockV5, SynchronousCalls)
  ...
  property("arg of the first function should NOT overlap var accessed from body of the second function AFTER fix") {
    ...
    withDomain(domainSettingsWithFS(settings(fix = false))) { d =>
      d.appendBlock(preparingTxs: _*)
      d.appendBlock(invoke())
      d.blockchain.accountData(dApp, "key").get.value shouldBe 1
    }
    withDomain(domainSettingsWithFS(settings(fix = true))) { d =>
      d.appendBlock(preparingTxs: _*)
      d.appendBlock(invoke())
