...
class SyncDAppTransferBalanceCheckTest extends PropSpec with WithDomain with TransactionGenBase {
  ...
  property("temporary negative balance of sync call transfers produce error only after set height") {
    val (preparingTxs, invoke, dApp2Address) = scenario.sample.get
    val settings =
