...
trait TransactionGenBase extends ScriptGen with TypedScriptGen with NTPTime { _: Suite =>
  ...
  def invokeScriptGen(paymentsGen: Gen[Seq[Payment]]): Gen[InvokeScriptTransaction] =
    for {
      ...
      .selfSigned(1.toByte, sender, dappAddress.toAddress, Some(fc), payments, fee, Waves, feeIncreaseFactor = InvokeScriptTransaction.DefaultFeeIncreaseFactor, timestamp)
      .explicitGet()
