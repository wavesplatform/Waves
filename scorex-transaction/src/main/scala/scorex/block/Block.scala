...
import scorex.transaction.{TransactionLayerRegistry, GenesisTransaction, LagonakiTransaction}
import TransactionLayerRegistry.blockchainStorage
...
case class Block(version: Byte,
                 ...
                 generationData: Constants.ConsensusAlgo.kernelData,
                 transactions: Seq[LagonakiTransaction],
  ...
  def parent(): Option[Block] = blockchainStorage.parent(this)

  def child(): Option[Block] = blockchainStorage.child(this)

  def height(): Option[Int] = blockchainStorage.heightOf(this)
...
object Block {
  ...
  val Version: Byte = 1
  val MaxBlockSizeInBytes = 1024 * 1024 // 1 mb block
  ...
  val MaxTransactionBytes = MaxBlockSizeInBytes - BaseLength

  def apply(stub: BlockStub, transactions: Seq[LagonakiTransaction], account: PrivateKeyAccount): Block =
  ...
  def apply(stub: BlockStub, account: PrivateKeyAccount): Block = {
    ...
    val (_, transactions) = orderedTransactions.foldLeft((0, List[LagonakiTransaction]())) {
  ...
  def parse(data: Array[Byte]): Try[Block] = Try {
    ...
    if (generationData.isGenesis) {
      ...
      val (sigPosition, transactions) = (1 to transactionCount).foldLeft((position, Seq[LagonakiTransaction]())) { case ((pos, txs), _) =>
  ...
  def isNewBlockValid(block: Block) =
    block != ConsensusAlgo.genesisBlock &&
      ...
      blockchainStorage.lastBlock.signature.sameElements(block.reference) &&
      block.isValid()
}
