...
object MatcherKeys {
  ...
  def order(orderId: ByteStr): Key[Option[Order]] = Key.opt(
    "matcher-order",
    (1.toShort, orderId.arr),
    xs => Order.fromBytes(xs.head, xs.tail),
    o => o.version +: o.bytes()
  )

  def orderInfo(orderId: ByteStr): Key[Option[FinalOrderInfo]] =
    Key.opt("matcher-order-info", (2.toShort, orderId.arr), OrderInfo.decode, OrderInfo.encode)
  ...
  def orderTxId(orderId: ByteStr, seqNr: Int): Key[ByteStr] = Key("matcher-order-tx-id", (10.toShort, orderId.arr, seqNr), ByteStr(_), _.arr)

  def exchangeTransaction(txId: ByteStr): Key[Option[ExchangeTransaction]] =
    Key.opt("matcher-exchange-transaction", (11.toShort, txId.arr), ExchangeTransaction.parse(_).get, _.bytes())
  ...
  def finalizedCommon(address: Address, seqNr: Int): Key[Option[Order.Id]] =
    Key.opt("matcher-finalized-common", (15.toShort, address.bytes.arr ++ Ints.toByteArray(seqNr)), ByteStr(_), _.arr)
  ...
  def finalizedPair(address: Address, pair: AssetPair, seqNr: Int): Key[Option[Order.Id]] =
    Key.opt("matcher-finalized-pair", (17.toShort, address.bytes.arr ++ pair.bytes ++ Ints.toByteArray(seqNr)), ByteStr(_), _.arr)

  def lastCommandTimestamp(address: Address): Key[Option[Long]] =
    Key.opt("matcher-last-command-timestamp", (18.toShort, address.bytes.arr), Longs.fromByteArray, Longs.toByteArray)
  ...
  def lpqElement(idx: Long): Key[Option[QueueEventWithMeta]] =
    Key.opt(
      LqElementKeyName,
      (LqElementPrefix, Longs.toByteArray(idx)),
